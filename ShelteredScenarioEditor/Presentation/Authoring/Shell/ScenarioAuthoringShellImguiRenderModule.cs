...
namespace ShelteredScenarioEditor.Presentation.Authoring.Shell{
    internal sealed partial class ScenarioAuthoringShellImguiRenderModule
    {
        ...
        private readonly ScenarioAuthoringTourTargetRegistry _tourTargets;
        ...
        public ScenarioAuthoringShellImguiRenderModule(
            ScenarioAuthoringShellAnimationService animations,
            ...
            ScenarioAuthoringTourTargetRegistry tourTargets,
        ...
        public void Render(ScenarioAuthoringPresentationSnapshot snapshot)
        {
            ...
            bool vanillaBlockingPanelOpen = _panelVisibility != null && _panelVisibility.HasBlockingPanelOpen();
        ...
        private void Draw()
        {
            ...
            ScenarioAuthoringInputCaptureService inputCapture = _inputCapture;
            if (inputCapture == null)
                return;
            inputCapture.BeginFrame(uiScale);
            ScenarioAuthoringTourTargetRegistry tourTargets = _tourTargets;
            if (tourTargets != null)
            ...
            _topBarMoreMenuOpen = _rendererInteraction.TopBarMoreOpen;
            ...
            {
                _backend.ExecuteCommand(ShellUxCommand.Simple(ShellUxCommandKind.ToggleWindowMenu, ScenarioAuthoringActionIds.ActionShellToggleWindowMenu));
            ...
            {
                ExecuteRendererCommand(RendererInteractionCommand.ForSimple(RendererInteractionCommandKind.ToggleTopBarMore, RendererInteractionAutomationIds.ActionRendererTopBarMoreToggle));
                _topBarMoreMenuOpen = _rendererInteraction.TopBarMoreOpen;
                Event.current.Use();
            }
            ...
            {
                inputCapture.CompleteFrame(_snapshot != null ? _snapshot.State : null);
        ...
        {
            if (row == null || row.ApplyColorCommand == null)
                return;

            CloseSurvivorColorPicker();
            ModColor initial = new ModColor(row.Color.r, row.Color.g, row.Color.b, row.Color.a);
            CharacterEditorCommand applyCommand = row.ApplyColorCommand;
            ...
            options.OnCommit = delegate(ModColor color, ColorPickerCommitKind commitKind)
            {
                ...
                _backend.ExecuteCommand(applyCommand.WithTextValue(color.ToHexRgba()));
            };
        ...
        private void CommitFloatingWindowFrame(string windowId, Rect rect, bool persist)
        {
            ...
            try
            {
                _backend.UpdateWindowFrame(windowId, rect.x, rect.y, rect.width, rect.height, persist);
            }
            catch
            {
            }
        ...
        private void BringFloatingWindowToFront(string windowId)
        {
            if (string.IsNullOrEmpty(windowId))
                return;
            ...
            try
            {
                _backend.BringWindowToFront(windowId);
            }
            catch
            {
            }
        ...
        private ScenarioAssetPlaceflowPhase ResolvePlaceflowPhase(bool toolWorkspaceOpen)
        {
        ...
        private bool IsBuildPlacementActive()
        {
            try
            {
                return _buildPlacement != null && _buildPlacement.HasActivePlacement;
            }
            catch
            {
                return false;
        ...
        private bool IsSceneSpritePlacementActive()
        {
            try
            {
                return _sceneSpritePlacement != null && _sceneSpritePlacement.HasActivePlacement;
            }
            catch
            {
                return false;
