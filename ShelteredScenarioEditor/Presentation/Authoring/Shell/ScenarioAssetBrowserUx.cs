...
namespace ShelteredScenarioEditor.Presentation.Authoring.Shell
{
    internal static class ScenarioAssetBrowserUx
    {
        ...
        internal const string RecentKey = "asset_browser.recent";
        ...
        internal static bool ToggleFavorite(
            ScenarioAuthoringState state,
            ...
            ScenarioAuthoringSettingsService settingsService)
        {
            ...
            state.Settings.Set(FavoritesKey, SerializeList(values));
            if (settingsService != null)
                settingsService.Save(state.Settings);
            return favorite;
        }

        internal static void RecordRecent(
            ScenarioAuthoringState state,
            string sourceActionId,
            ScenarioAuthoringSettingsService settings)
        {
            if (state == null || state.Settings == null || string.IsNullOrEmpty(sourceActionId))
                return;

            List<string> values = ReadList(state, RecentKey);
            int existingIndex = IndexOf(values, sourceActionId);
            if (existingIndex >= 0)
            ...
            state.Settings.Set(RecentKey, SerializeList(values));
            if (settings != null)
            ...
            string filter)
        {
            if (action == null)
                return false;
            string sourceActionId = GetSourceAutomationId(action);
        ...
        internal static string GetSourceAutomationId(ScenarioAuthoringInspectorAction action)
        {
            AssetBrowserCommand browser = action != null ? action.Command as AssetBrowserCommand : null;
            return browser != null && browser.Kind == AssetBrowserCommandKind.Select && browser.Selection != null
                ? browser.Selection.AutomationId
                : null;
        }
        ...
        private static string FindSectionForAction(ScenarioAuthoringInspectorSection[] sections, string sourceActionId)
        {
            ...
            for (int i = 0; sections != null && i < sections.Length; i++)
            {
                ...
                {
                    ScenarioAuthoringInspectorAction action = section.Items[j] != null ? section.Items[j].Action : null;
                    if (action != null && string.Equals(GetSourceAutomationId(action), sourceActionId, StringComparison.Ordinal))
                        return section.Id;
                }
            }
            return null;
