...
namespace ShelteredScenarioEditor.Presentation.Authoring.Shell{
    ...
    // BuildPresetPreviewDocument -> This preset sets -> typed preset apply command
    /// <summary>
    ...
    internal static class ScenarioSuppliesAuthoringContentBuilder
    {
        public static ScenarioAuthoringInspectorSection BuildStartingItemsSection(StartingInventoryDefinition inventory)
        {
            ...
            items.Add(Item.ActionItem(Item.Action(
                GameplayScheduleCommands.MergeStartingDuplicates(),
                "Merge Duplicates",
            ...
            items.Add(Item.ActionItem(Item.Action(
                GameplayScheduleCommands.ToggleStartingOverride(),
        ...
        private static ScenarioInventorySlotGridViewModel BuildStartingInventorySlotGrid(StartingInventoryDefinition inventory)
        {
            ...
            for (int i = 0; inventory != null && inventory.Items != null && i < inventory.Items.Count; i++)
            {
                ...
                {
                    Id = "starting." + index.ToString(CultureInfo.InvariantCulture),
                    ...
                    PrimaryAction = Item.Action(
                        GameplayScheduleCommands.OpenStartingPicker(index),
                    ...
                    QuantityIncreaseAction = Item.Action(GameplayScheduleCommands.StepStartingQuantity(index, 1), "+", "Increase this starting stack by one.", true, false, "+"),
                    QuantityDecreaseAction = Item.Action(GameplayScheduleCommands.StepStartingQuantity(index, -1), "-", "Decrease this starting stack by one.", true, false, "-"),
                    RemoveAction = Item.Action(GameplayScheduleCommands.RemoveStartingItem(index), "Remove", "Remove this starting stack.", true, false, "RM")
                });
            }
        ...
        private static void AddEmptyAddSlot(List<ScenarioInventorySlotViewModel> slots)
        {
            ...
            slots.Add(new ScenarioInventorySlotViewModel
            {
                ...
                PrimaryAction = Item.Action(
                    GameplayScheduleCommands.AddStartingItemAndPick(),
                    "Add Starting Item",
        ...
        public static ScenarioAuthoringInspectorSection BuildBalanceSection(ScenarioDefinition definition, StartingInventoryDefinition inventory)
        {
            ...
            items.Add(Item.Text(ScenarioSuppliesBalanceEstimator.AssumptionsLine()));

            string capacityNote;
            if (TryBuildCapacityNote(inventory, out capacityNote))
                items.Add(Item.Text(capacityNote));
        ...
        public static ScenarioAuthoringInspectorSection[] BuildPresetDocumentSections(ScenarioDefinition definition, int presetIndex)
        {
            ...
            if (stacks.Count == 0)
            {
                stackItems.Add(Item.Text("This preset clears all starting items."));
            }
            else
            {
            ...
            {
                noteItems.Add(Item.Text("No starting items are authored yet, so nothing is overwritten."));
            }
            ...
            noteItems.Add(Item.Property(
                "Water",
                ScenarioSuppliesBalanceEstimator.FormatDays(estimate.WaterDays)));
            noteItems.Add(Item.Property(
                "Food",
                ScenarioSuppliesBalanceEstimator.FormatDays(estimate.FoodDays)));
            noteItems.Add(Item.Text(ScenarioSuppliesBalanceEstimator.AssumptionsLine()));
            noteItems.Add(Item.ActionItem(Item.Action(
                GameplayScheduleCommands.ApplySuppliesPreset(presetIndex),
