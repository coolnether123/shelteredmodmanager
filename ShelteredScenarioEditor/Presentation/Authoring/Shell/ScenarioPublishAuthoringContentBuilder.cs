...
namespace ShelteredScenarioEditor.Presentation.Authoring.Shell{
    internal sealed class ScenarioPublishAuthoringContentBuilder : IScenarioAuthoringWindowContentBuilder
    {
        ...
        public ScenarioPublishAuthoringContentBuilder(
            ...
            ScenarioPreviewSessionHost previewSession = null)
        {
        ...
        public ScenarioAuthoringInspectorSection[] Build(ScenarioAuthoringWindowContentContext context)
        {
            ...
            ScenarioAuthoringValidationSnapshot validation = EvaluateValidation(_validator, scenarioFilePath, definition);
            ...
            List<ScenarioAuthoringInspectorItem> timelineItems = BuildTimelineItems(definition, GetRuntimeState(_previewSession), _timelineBuilder, true);
            List<ScenarioAuthoringInspectorItem> validationItems = BuildValidationItems(validation, scenarioFilePath);
            bool showAdvanced = state != null
                && state.Settings != null
                && state.Settings.ShowAdvancedDetails;
            List<ScenarioAuthoringInspectorItem> exportItems = BuildExportItems(editorSession, definition, validation, showAdvanced);
            List<ScenarioAuthoringInspectorItem> packageItems = BuildPackagePreviewItems(state, validation, scenarioFilePath);
            ...
            metadataItems.AddRange(ScenarioMetadataAuthoringContent.BuildStatusItems(scenarioFilePath, _publishService));
            return new[]
            {
        ...
        internal ScenarioAuthoringInspectorSection[] BuildStatusFlowSections(ScenarioAuthoringWindowContentContext context)
        {
            ...
            ScenarioAuthoringInspectorSection blockers = ScenarioAuthoringStatusFlowSupport.Section(
                "publish_validation",
                "Resolve blockers",
                BuildValidationItems(validation, scenarioFilePath),
            ...
            ScenarioAuthoringInspectorSection contents = ScenarioAuthoringStatusFlowSupport.Section(
                ...
                BuildPackagePreviewItems(state, validation, scenarioFilePath),
                ScenarioAuthoringInspectorSectionLayout.PropertyList);
            ...
            ScenarioAuthoringInspectorSection export = ScenarioAuthoringStatusFlowSupport.Section(
                "publish_export",
                "Export",
                BuildExportControlItems(editorSession, definition, validation),
                ScenarioAuthoringInspectorSectionLayout.PropertyList);
        ...
        internal static ScenarioAuthoringValidationSnapshot EvaluateValidation(
        ...
        internal static List<ScenarioAuthoringInspectorItem> BuildValidationItems(ScenarioAuthoringValidationSnapshot validation, string scenarioFilePath)
        {
            ...
            items.Add(Item.ActionItem(Item.Action(EditorLifecycleCommand.SaveDraft, "Save / Revalidate", "Run validation and save the current draft.", true, true, "SV")));
            ...
            for (int i = 0; i < issues.Length && i < 16; i++)
            {
                ...
                if (issue != null)
                {
                    ...
                    if (issue.Severity == ScenarioIssueSeverity.Warning)
                    {
                        if (accepted != null)
                        {
                            items.Add(Item.ActionItem(Item.Action(ScenarioPublishCommand.UnacceptWarning(fingerprint), "Review Again", "Remove this warning acceptance.", true, false, "WA")));
                        }
                        else
                        {
                            ...
                            note.Editable = true;
                            note.Action = Item.Action(ScenarioPublishCommand.AcceptWarning(fingerprint, string.Empty), "Accept Warning", "Commit a short acceptance note.", true, false, "WA");
                            items.Add(note);
                        }
                    }
                    if (accepted != null)
        ...
        internal List<ScenarioAuthoringInspectorItem> BuildExportItems(
            ScenarioEditorSession editorSession,
            ScenarioDefinition definition,
            ScenarioAuthoringValidationSnapshot validation,
            bool showAdvanced)
        {
            List<ScenarioAuthoringInspectorItem> items = BuildExportControlItems(editorSession, definition, validation);
        ...
        private List<ScenarioAuthoringInspectorItem> BuildExportResultItems(bool showAdvanced)
        {
            List<ScenarioAuthoringInspectorItem> items = new List<ScenarioAuthoringInspectorItem>();
            ...
            if (!string.IsNullOrEmpty(last.ArtifactRootPath))
            {
                if (showAdvanced)
                {
                    ...
                    items.Add(Item.ActionItem(Item.Action(ScenarioPublishCommand.OpenLastExportFolder(), "Open Export Folder", "Open the last export folder in Windows Explorer.", true, false, "OP", last.ArtifactRootPath)));
                    items.Add(Item.ActionItem(Item.Action(ScenarioPublishCommand.CopyLastExportPath(), "Copy Path", "Copy the last export folder path to the clipboard.", true, false, "CP", last.ArtifactRootPath)));
                }
                items.Add(Item.ActionItem(Item.Action(ScenarioPublishCommand.InstallLastExport(false), "Install this export locally", "Copy this validated package into the user scenario library and refresh the scenario catalog.", last.Success, false, "IN")));
                ScenarioPackageOperationResult install = GetLastInstallResult();
                if (install != null && install.ConfirmationRequired)
                    items.Add(Item.ActionItem(Item.Action(ScenarioPublishCommand.InstallLastExport(true), "Confirm same-ID replacement", install.Message, true, true, "!!")));
                if (install != null && install.Success && !string.IsNullOrEmpty(install.InstallPath))
                    items.Add(Item.ActionItem(Item.Action(ScenarioPublishCommand.UninstallLastExport(), "Uninstall local test copy", "Remove only the locally installed copy of this export.", true, false, "UN")));
            }
        ...
        private ScenarioAuthoringStatusChipViewModel BuildLastExportStatusChip()
        {
            ScenarioPublishExportResult last = GetLastExportResult();
            if (last == null)
        ...
        internal List<ScenarioAuthoringInspectorItem> BuildPackagePreviewItems(
            ...
            string scenarioFilePath)
        {
            ...
            try
            {
                plan = _publishService != null ? _publishService.PreviewActiveDraft(state) : null;
            }
            catch (Exception ex)
            {
            ...
            if (plan == null)
            {
                items.Add(Item.ActionItem(Item.Action(ScenarioPublishCommand.ToggleReadme(), "README.txt: " + (preferences.IncludeReadme ? "Included" : "Excluded"), "Toggle the human-readable install guide in the package.", true, false, "RD")));
                return items;
            }
        ...
        internal static ScenarioAuthoringInspectorAction BuildIssueNavigationAction(ScenarioValidationIssue issue)
        {
            ...
            return Item.Action(
                ShellUxCommand.SelectStage(stage),
        ...
        private static ScenarioAuthoringInspectorAction BuildIssueTopicAction(string issueTopic)
        {
            ...
            return Item.Action(
                ShellUxCommand.HelpTopic(issueTopic),
                "How to Fix This",
            ...
            ScenarioModCompatibilityReport compatibilityReport)
        {
            ...
            var compatibility = ShelteredScenarios.CheckScenarioDefBuilderCompatibility();
        ...
        private ScenarioPublishExportResult GetLastExportResult()
        {
            return _publishService != null ? _publishService.LastResult : null;
        }

        private ScenarioPackageOperationResult GetLastInstallResult()
        {
            return _publishService != null ? _publishService.LastInstallResult : null;
        }
        ...
        internal static List<ScenarioAuthoringInspectorItem> BuildTimelineItems(
            ScenarioDefinition definition,
            ScenarioRuntimeSnapshot runtimeState,
            ScenarioTimelineBuilder timelineBuilder,
        ...
        internal static ScenarioRuntimeSnapshot GetRuntimeState(ScenarioPreviewSessionHost previewSession)
        {
            try
            {
    ...
    internal sealed class ScenarioRuntimeTestAuthoringContentBuilder : IScenarioAuthoringWindowContentBuilder
    {
        ...
        private readonly ScenarioAuthorTestChecklistSectionBuilder _testChecklistSectionBuilder;
        private readonly IScenarioDefinitionValidator _validator;
        private readonly ScenarioTestConsoleService _testConsoleService;
        private readonly ScenarioPreviewSessionHost _previewSession;
        ...
        public ScenarioRuntimeTestAuthoringContentBuilder(
            ...
            ScenarioModCompatibilityViewModelBuilder modCompatibilityViewModelBuilder,
            IScenarioDefinitionValidator validator = null,
            ScenarioTestConsoleService testConsoleService = null,
            ScenarioPreviewSessionHost previewSession = null)
        {
            _timelineBuilder = timelineBuilder;
            _modDependencyDetector = modDependencyDetector;
            _modCompatibilityViewModelBuilder = modCompatibilityViewModelBuilder;
            _validator = validator;
            _testConsoleService = testConsoleService;
            _previewSession = previewSession;
            _testChecklistSectionBuilder = new ScenarioAuthorTestChecklistSectionBuilder(new ScenarioAuthorTestChecklistService());
        }
        ...
        public ScenarioAuthoringInspectorSection[] Build(ScenarioAuthoringWindowContentContext context)
        {
            ...
            ScenarioDefinition definition = context != null ? context.Definition : null;
            ScenarioAuthoringSession authoringSession = context != null ? context.Session : null;
            ScenarioAuthoringValidationSnapshot validation = ScenarioPublishAuthoringContentBuilder.EvaluateValidation(
                _validator,
                authoringSession != null ? authoringSession.ScenarioFilePath : null,
                definition);
            ...
            List<ScenarioAuthoringInspectorItem> resultItems = BuildPlaytestResultItems(editorSession, _previewSession);
            List<ScenarioAuthoringInspectorItem> journalItems = BuildRuntimeJournalItems(_previewSession);
            List<ScenarioAuthoringInspectorItem> pendingItems = ScenarioPublishAuthoringContentBuilder.BuildTimelineItems(definition, ScenarioPublishAuthoringContentBuilder.GetRuntimeState(_previewSession), _timelineBuilder, false);
            ...
            List<ScenarioAuthoringInspectorSection> sections = new List<ScenarioAuthoringInspectorSection>
            {
                ...
                _testChecklistSectionBuilder.BuildTestSection(editorSession),
                new ScenarioAuthoringInspectorSection
                {
            ...
            if (editorSession != null && editorSession.PlaytestState == ScenarioPlaytestState.Playtesting)
                sections.AddRange(ScenarioTestConsoleAuthoringContentBuilder.Build(context, _testConsoleService, _previewSession));
        ...
        internal ScenarioAuthoringInspectorSection[] BuildStatusFlowSections(ScenarioAuthoringWindowContentContext context)
        {
            ...
            bool showAdvanced = authoringState != null
                ...
                && authoringState.Settings.ShowAdvancedDetails;
            ...
            ScenarioAuthoringInspectorSection checklist = _testChecklistSectionBuilder.BuildTestSection(editorSession);
            ...
            ScenarioAuthoringInspectorSection[] consoleSections = ScenarioTestConsoleAuthoringContentBuilder.Build(context, _testConsoleService, _previewSession);
            ...
            List<ScenarioAuthoringInspectorItem> resultAndLogItems = BuildPlaytestResultItems(editorSession, _previewSession);
            if (log.Items != null)
            ...
            if (showAdvanced)
            {
                ...
                advancedItems.AddRange(BuildRuntimeJournalItems(_previewSession));
                advancedItems.AddRange(ScenarioPublishAuthoringContentBuilder.BuildTimelineItems(
                    definition,
                    ScenarioPublishAuthoringContentBuilder.GetRuntimeState(_previewSession),
                    _timelineBuilder,
                    false));
            ...
            ScenarioAuthoringValidationSnapshot validation)
        {
            ...
            bool canStartFromCast = ShelteredScenarioAuthoring.CanStartPlay(definition, out playStartReason);
            ...
            {
                items.Add(Item.ActionItem(Item.Action(
                    EditorLifecycleCommand.RestartPlaytest,
                    "Restart",
        ...
        private static List<ScenarioAuthoringInspectorItem> BuildRunSettingItems(ScenarioDefinition definition)
        {
            ...
            items.Add(Item.ActionItem(Item.Action(
                EditorLifecycleCommand.UseRandomSeed,
                "Random",
                ...
                "RD")));
            items.Add(Item.ActionItem(Item.Action(
                EditorLifecycleCommand.UseFixedSeed,
                "Fixed",
            ...
            seedItem.Action = Item.Action(
                EditorLifecycleCommand.Seed(string.Empty),
                "Set Seed",
                seedItem.HoverHint,
                fixedSeed,
                false,
            ...
            items.Add(Item.ActionItem(Item.Action(
                EditorLifecycleCommand.RerollSeed,
        ...
        private static List<ScenarioAuthoringInspectorItem> BuildRuntimeJournalItems(ScenarioPreviewSessionHost previewSession)
        {
            List<ScenarioAuthoringInspectorItem> items = new List<ScenarioAuthoringInspectorItem>();
            ScenarioRuntimeSnapshot state = ScenarioPublishAuthoringContentBuilder.GetRuntimeState(previewSession);
            ...
            items.Add(Item.Property("Outcome", Item.Safe(state != null ? state.Outcome : null)));
            if (state != null && !string.IsNullOrEmpty(state.OutcomeConditionId))
                items.Add(Item.Property("Outcome Condition", state.OutcomeConditionId));
            ...
            int count = state != null && state.Actions != null ? state.Actions.Length : 0;
            items.Add(Item.Property("Executed Actions", count.ToString()));
            for (int i = 0; state != null && state.Actions != null && i < state.Actions.Length && i < 8; i++)
            {
                ScenarioRuntimeActionSnapshot record = state.Actions[i];
                if (record != null)
                    items.Add(Item.Property(Item.Safe(record.ActionKey), record.Status + " / day " + record.Day + " " + record.Hour.ToString("D2") + ":" + record.Minute.ToString("D2")));
            }
            return items;
        }

        private static List<ScenarioAuthoringInspectorItem> BuildPlaytestResultItems(
            ScenarioEditorSession editorSession,
            ScenarioPreviewSessionHost previewSession)
        {
            ...
            items.Add(Item.Property(
                ...
                ScenarioAuthoringDisplayNameResolver.ShellRebuild.Resolve(
                    ...
                    state.Outcome,
                    "Unresolved").Text));
            items.Add(Item.Property("World Changes", Count(state.Actions).ToString(CultureInfo.InvariantCulture) + " actions / " + Count(state.Flags).ToString(CultureInfo.InvariantCulture) + " flags"));
            int failed = CountRecords(state, "Failed");
            int blocked = CountRecords(state, "Blocked");
            ...
            AddRuntimeRecordItems(items, state, "Failed", "Failed");
            AddRuntimeRecordItems(items, state, "Blocked", "Blocked");
            return items;
        }

        private static void AddRuntimeRecordItems(
            List<ScenarioAuthoringInspectorItem> items,
            ScenarioRuntimeSnapshot state,
            string status,
            string label)
        {
            int added = 0;
            for (int i = 0; state != null && state.Actions != null && i < state.Actions.Length && added < 4; i++)
            {
                ScenarioRuntimeActionSnapshot record = state.Actions[i];
                if (record == null || !string.Equals(record.Status, status, StringComparison.Ordinal))
                    continue;
        ...
        private static int CountRecords(ScenarioRuntimeSnapshot state, string status)
        {
            int count = 0;
            for (int i = 0; state != null && state.Actions != null && i < state.Actions.Length; i++)
            {
                if (state.Actions[i] != null && string.Equals(state.Actions[i].Status, status, StringComparison.Ordinal))
                    count++;
            }
    ...
    internal sealed class ScenarioTimelineAuthoringContentBuilder : IScenarioAuthoringWindowContentBuilder
    {
        ...
        public ScenarioAuthoringInspectorSection[] Build(ScenarioAuthoringWindowContentContext context)
        {
            ScenarioAuthoringState state = context != null ? context.State : null;
            ScenarioDefinition definition = context != null ? context.Definition : null;
            ScenarioRuntimeSnapshot runtimeState = ScenarioPublishAuthoringContentBuilder.GetRuntimeState(_previewSession);
        ...
        private static void AddTimelinePresetActions(List<ScenarioAuthoringInspectorItem> items, ScenarioDefinition definition)
        {
            ...
            items.Add(Item.ActionItem(Item.Action(EventAuthoringCommand.Create(EventAuthoringOperation.AddScheduledAction), "Custom change", "Create an empty scheduled change when none of the common plans fit.", true, false, "A+", "Advanced")));
        }
        ...
        private static void AddPreset(List<ScenarioAuthoringInspectorItem> items, ScenarioDefinition definition, string presetId, string label, string hint, string icon)
        {
            ...
            ScenarioAuthoringInspectorAction action = Item.Action(EventAuthoringCommand.Create(EventAuthoringOperation.ApplyTimelinePreset, value: presetId), label, hint, enabled, true, icon, "Preset");
            if (!enabled)
        ...
        {
            ScenarioAuthoringInspectorAction action = Item.Action(
                TimelineNavigationCommand.SelectDay(day.ToString(CultureInfo.InvariantCulture)),
        ...
        private static ScenarioAuthoringInspectorAction TimelineChipAction(ScenarioAuthoringState state, ScenarioDefinition definition, ScenarioTimelineEntry entry)
        {
            ...
            ScenarioAuthoringInspectorAction action = Item.Action(
                TimelineNavigationCommand.OpenEntry(entry != null ? entry.Id : string.Empty),
                label,
                hint,
                true,
                emphasized,
        ...
        private static List<ScenarioAuthoringInspectorItem> BuildTimelineLogicItems(ScenarioDefinition definition)
        {
            ...
            items.Add(Item.ActionItem(Item.Action(EventAuthoringCommand.Create(EventAuthoringOperation.AddManualTrigger), "Add Manual Trigger", "Create a trigger fired by code or another scheduled effect.", true, false, "T+")));
            items.Add(Item.ActionItem(Item.Action(EventAuthoringCommand.Create(EventAuthoringOperation.AddGate), "Add Condition", "Create a reusable condition for scheduled changes.", true, false, "C+")));
            return items;
        }
        ...
        private static int CountManualTriggers(ScenarioDefinition definition)
        {
            ...
            for (int i = 0; definition != null && definition.TriggersAndEvents != null && definition.TriggersAndEvents.Triggers != null && i < definition.TriggersAndEvents.Triggers.Count; i++)
                if (ShelteredScenarioAuthoring.IsManualTrigger(definition.TriggersAndEvents.Triggers[i]))
                    count++;
