...
namespace ShelteredScenarioEditor.Presentation.Authoring.Shell{
    internal sealed class ScenarioHierarchyAuthoringContentBuilder : IScenarioAuthoringWindowContentBuilder
    {
        ...
        public ScenarioHierarchyAuthoringContentBuilder(ScenarioAuthoringSelectionService selectionService)
        {
        ...
        public ScenarioAuthoringInspectorSection[] Build(ScenarioAuthoringWindowContentContext context)
        {
            ...
            if (context != null && context.BackdropSections != null && context.BackdropSections.Length > 0)
                sections.Add(context.BackdropSections[0]);
        ...
        private ScenarioAuthoringInspectorSection BuildBunkerSection(ScenarioAuthoringState state, ScenarioDefinition definition)
        {
        ...
        private ScenarioAuthoringInspectorSection BuildLiveObjectsSection(ScenarioAuthoringState state)
        {
            List<ScenarioAuthoringInspectorItem> items = new List<ScenarioAuthoringInspectorItem>();
        ...
        private ScenarioAuthoringInspectorSection BuildCharactersSection(ScenarioAuthoringState state, ScenarioDefinition definition)
        {
            List<ScenarioAuthoringInspectorItem> items = new List<ScenarioAuthoringInspectorItem>();
            ...
            for (int i = 0; definition != null && definition.FamilySetup != null && definition.FamilySetup.Members != null && i < definition.FamilySetup.Members.Count && i < 8; i++)
            {
                ...
                items.Add(Item.ActionItem(BuildNavigationAction(
                    ToolCommand.Select(ScenarioAuthoringTool.Family),
                    label,
        ...
        private static ScenarioAuthoringInspectorSection BuildEventSection(ScenarioAuthoringState state, ScenarioDefinition definition)
        {
            ...
            for (int i = 0; definition != null && definition.TriggersAndEvents != null && definition.TriggersAndEvents.Triggers != null && i < definition.TriggersAndEvents.Triggers.Count && i < 4; i++)
            {
                ...
                items.Add(Item.ActionItem(BuildNavigationAction(
                    ShellUxCommand.Simple(ShellUxCommandKind.OpenTimeline, ScenarioAuthoringActionIds.ActionShellOpenTimeline),
        ...
        private static ScenarioAuthoringInspectorSection BuildAssetSection(ScenarioAuthoringState state, ScenarioDefinition definition)
        {
            ...
            for (int i = 0; definition != null && definition.AssetReferences != null && definition.AssetReferences.SceneSpritePlacements != null && i < definition.AssetReferences.SceneSpritePlacements.Count && i < 6; i++)
            {
                ...
                items.Add(Item.ActionItem(BuildNavigationAction(
                    ShellUxCommand.SelectStage(ScenarioStageKind.Assets),
        ...
        private ScenarioAuthoringInspectorAction BuildObjectPlacementAction(ScenarioAuthoringState state, ObjectPlacement placement, int index)
        {
            ...
            GameObject runtimeObject = ScenarioRuntimeIdentityCatalog.FindObjectPlacement(id);
            if (runtimeObject != null)
                return BuildTargetAction(state, runtimeObject, ScenarioAuthoringTargetKind.PlaceableObject, "OBJ", placement.ScenarioObjectId);

            return BuildNavigationAction(
                ShellUxCommand.SelectStage(ScenarioStageKind.Bunker),
                Item.Safe(label),
        ...
        private ScenarioAuthoringInspectorAction BuildTargetAction(ScenarioAuthoringState state, GameObject gameObject, ScenarioAuthoringTargetKind kind, string badge)
        {
            return BuildTargetAction(state, gameObject, kind, badge, null);
        }

        private ScenarioAuthoringInspectorAction BuildTargetAction(ScenarioAuthoringState state, GameObject gameObject, ScenarioAuthoringTargetKind kind, string badge, string scenarioReferenceId)
        {
            ScenarioAuthoringTarget target = null;
            bool targetAvailable = _selectionService != null
                && _selectionService.TryCreateTarget(gameObject, out target)
                && target != null;
            if (!targetAvailable)
                target = null;

            ScenarioAuthoringTargetKind targetKind = target != null ? target.Kind : kind;
            GameObject runtimeTargetObject = target != null ? target.RuntimeObject as GameObject : null;
            GameObject targetObject = runtimeTargetObject != null ? runtimeTargetObject : gameObject;
            string targetReferenceId = !string.IsNullOrEmpty(scenarioReferenceId)
                ? scenarioReferenceId
                : (target != null ? target.ScenarioReferenceId : null);
            string id = target != null ? target.Id : string.Empty;
            string label = target != null
                ? target.DisplayName
                : ScenarioWorldObjectDisplayNameResolver.Resolve(gameObject, kind);
            bool activeSelected = targetAvailable && IsTargetSelected(state, id, targetReferenceId, targetObject);
            bool selected = activeSelected || (targetAvailable && IsTargetInMultiSelection(state, id, targetReferenceId, targetObject));
            bool hovered = !selected && targetAvailable && IsTargetHovered(state, id, targetReferenceId, targetObject);
            bool showAdvancedDetails = ShowAdvancedDetails(state);
            return Item.Action(
                SelectionCommand.SelectHierarchy(id),
                label,
                FormatRowDetail(FormatGameObjectConcept(targetObject, targetKind, showAdvancedDetails), activeSelected, selected, hovered),
                targetAvailable,
                activeSelected,
        ...
        private static ScenarioAuthoringInspectorAction BuildNavigationAction(ScenarioAuthoringCommand command, string label, string detail, string badge, bool emphasized)
        {
            return Item.Action(command, Item.Safe(label), detail, true, emphasized, badge, detail);
        }
        ...
        private static bool IsSameTarget(ScenarioAuthoringTarget target, string id, string scenarioReferenceId, GameObject gameObject)
        {
            ...
            return gameObject != null
                ...
                && string.Equals(target.TransformPath, ShelteredScenarioRuntime.GetTransformPath(gameObject.transform), System.StringComparison.OrdinalIgnoreCase);
        }
        ...
        private static string FormatGameObjectConcept(GameObject gameObject, ScenarioAuthoringTargetKind kind, bool showAdvancedDetails)
        {
            ...
            if (showAdvancedDetails)
                parts.Add(ShelteredScenarioRuntime.GetTransformPath(gameObject.transform));
