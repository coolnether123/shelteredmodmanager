...
namespace ShelteredScenarioEditor.Presentation.Authoring.Shell
{
    internal sealed class ScenarioSurvivorWorkspaceDocumentBuilder
    {
            ...
            ScenarioAuthoringWorkspaceViewModelFactory factory)
        {
            ...
            {
                Action(CharacterEditorCommand.Step(CharacterEditorCommandKind.Move, CharacterMemberScope.Starting, index, null, -1), "Move Up", "Move this survivor earlier in the starting order.", index > 0, false, "UP", index > 0 ? null : "Already first."),
                Action(CharacterEditorCommand.Step(CharacterEditorCommandKind.Move, CharacterMemberScope.Starting, index, null, 1), "Move Down", "Move this survivor later in the starting order.", index + 1 < family.Members.Count, false, "DN", index + 1 < family.Members.Count ? null : "Already last."),
                Action(CharacterEditorCommand.Member(CharacterEditorCommandKind.Remove, CharacterMemberScope.Starting, index), "Remove", "Remove this survivor from the starting cast.", true, false, "RM", null)
            };
            document.Sections = BuildEditorSections(context != null ? context.State : null, member, index, CharacterMemberScope.Starting, null, "Starting", null);
            return document;
        }
            ...
            ScenarioAuthoringWorkspaceViewModelFactory factory)
        {
            ...
            document.HeaderActions = new[]
            {
                Action(GameplayScheduleCommands.RemoveFutureSurvivor(index), "Remove", "Remove this future arrival.", true, false, "RM", null)
            };
            string arrivalSummary = survivor != null
                ...
                : "Arrival needs setup";
            document.Sections = BuildEditorSections(context != null ? context.State : null, member, index, CharacterMemberScope.Future, arrivalSummary, "Future", survivor);
            return document;
        }
        ...
        internal static bool NeedsAttention(FamilyMemberConfig member)
        {
            ...
            for (int i = 0; member.Stats != null && i < member.Stats.Count; i++)
            {
                ...
                if (stat != null && (stat.Value < ShelteredScenarioEditor.Domain.People.ScenarioFamilyConfigurationPolicy.StatMin || stat.Value > ShelteredScenarioEditor.Domain.People.ScenarioFamilyConfigurationPolicy.StatMax))
                    return true;
            }
            return false;
        }
            ...
            FutureSurvivorDefinition futureSurvivor)
        {
            ...
            sections.Add(new ScenarioAuthoringInspectorSection
            {
                ...
                SurvivorEditor = BuildSurvivorEditorViewModel(state, member, index, scope, arrivalSummary, status)
            });

            if (futureSurvivor != null)
                sections.Add(BuildArrivalSection(futureSurvivor, index));

            ScenarioAuthoringInspectorSection modFields = BuildSurvivorModFieldsSection(member, scope, index);
            if (modFields != null)
        ...
        private static ScenarioAuthoringInspectorSection BuildArrivalSection(FutureSurvivorDefinition survivor, int index)
        {
            ...
            return new ScenarioAuthoringInspectorSection
            {
                ...
                Items = new[]
                {
                    ...
                    ScenarioInspectorItemFactory.ActionItem(Action(GameplayScheduleCommands.StepFutureDay(index, 1), "Day +", "Move this arrival one day later.", true, false, "D+", null)),
                    ScenarioInspectorItemFactory.ActionItem(Action(GameplayScheduleCommands.StepFutureDay(index, -1), "Day -", "Move this arrival one day earlier.", true, false, "D-", null)),
                    ScenarioInspectorItemFactory.ActionItem(Action(GameplayScheduleCommands.StepFutureHour(index, 1), "Hour +", "Move this arrival one hour later.", true, false, "H+", null)),
                    ScenarioInspectorItemFactory.ActionItem(Action(GameplayScheduleCommands.StepFutureHour(index, -1), "Hour -", "Move this arrival one hour earlier.", true, false, "H-", null)),
                    ScenarioInspectorItemFactory.ActionItem(Action(GameplayScheduleCommands.ToggleFutureAsk(index), "Change Join Mode", "Switch between asking to join and joining automatically.", true, survivor.AskToJoin, "AJ", null))
                }
            };
        }
            ...
            string status)
        {
            ...
            return new ScenarioSurvivorEditorViewModel
            {
                ...
                NameAction = Action(CharacterEditorCommand.Member(CharacterEditorCommandKind.CycleName, scope, index), displayName, "Cycle this survivor's name preset.", true, false, "NM", displayName),
                GenderAction = Action(CharacterEditorCommand.Member(CharacterEditorCommandKind.CycleGender, scope, index), "Gender: " + member.Gender.ToString(), "Cycle Any, Female, and Male.", true, false, "GN", member.Gender.ToString()),
                BodyAction = Action(CharacterEditorCommand.Member(CharacterEditorCommandKind.ToggleAdult, scope, index), FormatAgeBand(member), "Toggle the vanilla adult or child body mesh.", true, false, "BD", FormatBody(member, true)),
                TextureRows = BuildSurvivorTextureRows(member, scope, index),
                ColorRows = BuildSurvivorColorRows(member, scope, index),
                StatRows = BuildSurvivorStatRows(member, scope, index),
                ...
                TraitRows = BuildSurvivorTraitRows(member, scope, index, strengthTrait, weaknessTrait),
                ConditionRows = BuildSurvivorConditionRows(member, scope, index),
                ...
                UtilityActions = new[]
                {
                    Action(CharacterEditorCommand.Member(CharacterEditorCommandKind.RandomizePerson, scope, index), "Randomize", ScenarioSurvivorAuthoringOperations.RandomizeDisclosure, true, false, "RND", null),
                    Action(CharacterEditorCommand.Member(CharacterEditorCommandKind.Duplicate, scope, index), "Duplicate Person", ScenarioSurvivorAuthoringOperations.DuplicateDisclosure, true, false, "DUP", null),
                    Action(CharacterEditorCommand.Member(CharacterEditorCommandKind.RandomizeLook, scope, index), "Randomize Look", "Randomize head, top, bottom, and color choices.", true, false, "RLK", FormatAppearance(member)),
                    Action(CharacterEditorCommand.Member(CharacterEditorCommandKind.CopyIdentity, scope, index), "Copy Selected Identity", "Copy identity and appearance from the selected live family member.", canCopySelected, false, "ID", canCopySelected ? "Selected live family member" : null, copyReason),
                    Action(CharacterEditorCommand.Member(CharacterEditorCommandKind.CopyLook, scope, index), "Copy Selected Look", "Copy appearance from the selected live family member.", canCopySelected, false, "LK", FormatAppearance(member), copyReason),
                    Action(CharacterEditorCommand.Member(CharacterEditorCommandKind.ClearLook, scope, index), "Clear Look", "Clear stored mesh, texture, and color overrides.", true, false, "CL", FormatAppearance(member))
                },
        ...
        {
            string[] statIds = ShelteredScenarioEditor.Domain.People.ScenarioFamilyConfigurationPolicy.StatIds;
        ...
        private static ScenarioSurvivorTextureRowViewModel[] BuildSurvivorTextureRows(FamilyMemberConfig member, CharacterMemberScope scope, int index)
        {
            FamilyMemberAppearanceConfig appearance = member != null ? member.Appearance : null;
            return new[]
            {
                SurvivorTextureRow("Head", scope, index, ScenarioEditorCharacterTexturePart.Head, appearance),
                SurvivorTextureRow("Top", scope, index, ScenarioEditorCharacterTexturePart.Torso, appearance),
                SurvivorTextureRow("Bottom", scope, index, ScenarioEditorCharacterTexturePart.Legs, appearance)
            };
        }
            ...
            FamilyMemberAppearanceConfig appearance)
        {
            return new ScenarioSurvivorTextureRowViewModel
            {
                ...
                PreviousAction = Action(CharacterEditorCommand.Texture(scope, index, part, -1), "<", "Switch to the previous vanilla " + label.ToLowerInvariant() + " sprite.", true, false, "<", null),
                NextAction = Action(CharacterEditorCommand.Texture(scope, index, part, 1), ">", "Switch to the next vanilla " + label.ToLowerInvariant() + " sprite.", true, false, ">", null)
            };
        }

        private static ScenarioSurvivorColorRowViewModel[] BuildSurvivorColorRows(FamilyMemberConfig member, CharacterMemberScope scope, int index)
        {
            Color hair;
            Color skin;
            Color shirt;
            ...
            return new[]
            {
                SurvivorColorRow("Hair", "hair", scope, index, ScenarioEditorCharacterColorPart.Hair, hair),
                SurvivorColorRow("Skin", "skin", scope, index, ScenarioEditorCharacterColorPart.Skin, skin),
                SurvivorColorRow("Shirt", "shirt", scope, index, ScenarioEditorCharacterColorPart.Shirt, shirt),
                SurvivorColorRow("Pants", "pants", scope, index, ScenarioEditorCharacterColorPart.Pants, pants)
            };
        }

        private static ScenarioSurvivorColorRowViewModel SurvivorColorRow(string label, string commandPart, CharacterMemberScope scope, int index, ScenarioEditorCharacterColorPart part, Color color)
        {
            return new ScenarioSurvivorColorRowViewModel
            {
                Channel = commandPart,
                Label = label,
                Hex = ScenarioEditorCharacterAppearanceService.ToColorHex(color),
                Color = color,
                PreviousAction = Action(CharacterEditorCommand.StepColor(scope, index, part, -1), "<", "Switch to the previous vanilla " + label.ToLowerInvariant() + " color.", true, false, "<", null),
                NextAction = Action(CharacterEditorCommand.StepColor(scope, index, part, 1), ">", "Switch to the next vanilla " + label.ToLowerInvariant() + " color.", true, false, ">", null),
                OpenColorPickerCommand = CharacterEditorCommand.OpenColorPicker(commandPart),
                ApplyColorCommand = CharacterEditorCommand.ApplyColor(scope, index, part, color)
            };
        }

        private static ScenarioSurvivorStatRowViewModel[] BuildSurvivorStatRows(FamilyMemberConfig member, CharacterMemberScope scope, int index)
        {
            string[] statIds = ShelteredScenarioEditor.Domain.People.ScenarioFamilyConfigurationPolicy.StatIds;
            ...
            for (int i = 0; i < statIds.Length; i++)
            {
                ...
                bool canIncrease = displayValue < ShelteredScenarioEditor.Domain.People.ScenarioFamilyConfigurationPolicy.StatMax;
                bool canDecrease = displayValue > ShelteredScenarioEditor.Domain.People.ScenarioFamilyConfigurationPolicy.StatMin;
                rows[i] = new ScenarioSurvivorStatRowViewModel
                {
                    Id = statId,
                    Label = statId,
                    Value = displayValue,
                    Min = ShelteredScenarioEditor.Domain.People.ScenarioFamilyConfigurationPolicy.StatMin,
                    Max = ShelteredScenarioEditor.Domain.People.ScenarioFamilyConfigurationPolicy.StatMax,
                    RangeText = "1-20",
                    DecreaseAction = Action(CharacterEditorCommand.Step(CharacterEditorCommandKind.StepStat, scope, index, statId, -1), "-", "Decrease " + statId + ".", canDecrease, false, "-", displayValue.ToString(CultureInfo.InvariantCulture), canDecrease ? detail : "Stats are limited to 1-20."),
                    IncreaseAction = Action(CharacterEditorCommand.Step(CharacterEditorCommandKind.StepStat, scope, index, statId, 1), "+", "Increase " + statId + ".", canIncrease, false, "+", displayValue.ToString(CultureInfo.InvariantCulture), canIncrease ? detail : "Stats are limited to 1-20."),
                    TextAction = Action(CharacterEditorCommand.ValueCommand(CharacterEditorCommandKind.SetStat, scope, index, statId, displayValue.ToString(CultureInfo.InvariantCulture)), displayValue.ToString(CultureInfo.InvariantCulture), "Enter a " + statId + " value from 1 to 20.", true, false, "TX", displayValue.ToString(CultureInfo.InvariantCulture))
                };
            }
            return rows;
        }

        private static ScenarioSurvivorTraitRowViewModel[] BuildSurvivorTraitRows(FamilyMemberConfig member, CharacterMemberScope scope, int index, string strengthTrait, string weaknessTrait)
        {
            return new[]
            {
                SurvivorTraitRow(member, scope, index, true, strengthTrait),
                SurvivorTraitRow(member, scope, index, false, weaknessTrait)
            };
        }

        private static ScenarioSurvivorTraitRowViewModel SurvivorTraitRow(FamilyMemberConfig member, CharacterMemberScope scope, int index, bool strength, string value)
        {
            ...
            return new ScenarioSurvivorTraitRowViewModel
            {
                ...
                Value = value,
                PickerKey = CharacterEditorCommand.Trait(scope, index, strength, string.Empty).AutomationId,
                PreviousAction = Action(CharacterEditorCommand.CycleTrait(scope, index, strength, -1), "<", "Switch to the previous valid " + kind + " trait.", true, false, "<", null),
                NextAction = Action(CharacterEditorCommand.CycleTrait(scope, index, strength, 1), ">", "Switch to the next valid " + kind + " trait.", true, false, ">", null),
                PickerAction = Action(CharacterEditorCommand.Trait(scope, index, strength, value), value == "<none>" ? "Choose trait" : FormatTraitName(value), "Pick a " + kind + " trait with its vanilla effect.", true, false, strength ? "ST" : "WT", value),
                Options = BuildTraitOptions(member, scope, index, strength)
            };
        }

        private static ScenarioSurvivorTraitOptionViewModel[] BuildTraitOptions(FamilyMemberConfig member, CharacterMemberScope scope, int index, bool strength)
        {
            ...
            for (int i = 0; values != null && i < values.Length; i++)
            {
                ...
                options.Add(new ScenarioSurvivorTraitOptionViewModel
                {
                    ...
                    SelectAction = Action(CharacterEditorCommand.Trait(scope, index, strength, id), label, description, !conflicts, false, strength ? "ST" : "WT", id, conflicts ? "Blocked by the paired trait." : null)
                });
            }
            return options.ToArray();
        }

        private static ScenarioSurvivorConditionRowViewModel[] BuildSurvivorConditionRows(FamilyMemberConfig member, CharacterMemberScope scope, int index)
        {
            string[] conditionIds = ShelteredScenarioEditor.Domain.People.ScenarioFamilyConfigurationPolicy.ConditionIds;
            ...
            for (int i = 0; i < conditionIds.Length; i++)
            {
                ...
                int rawValue;
                int value = ShelteredScenarioEditor.Domain.People.ScenarioFamilyConfigurationPolicy.TryGetConditionValue(member, id, out rawValue) ? rawValue : 0;
                bool canDecrease = value > ShelteredScenarioEditor.Domain.People.ScenarioFamilyConfigurationPolicy.ConditionMin;
                bool canIncrease = value < ShelteredScenarioEditor.Domain.People.ScenarioFamilyConfigurationPolicy.ConditionMax;
                rows[i] = new ScenarioSurvivorConditionRowViewModel
                {
                    Id = id,
                    Label = FormatConditionLabel(id),
                    Value = value,
                    Min = ShelteredScenarioEditor.Domain.People.ScenarioFamilyConfigurationPolicy.ConditionMin,
                    Max = ShelteredScenarioEditor.Domain.People.ScenarioFamilyConfigurationPolicy.ConditionMax,
                    RangeText = "0-100",
                    HelpText = GetConditionHelp(id),
                    DecreaseAction = Action(CharacterEditorCommand.Step(CharacterEditorCommandKind.StepCondition, scope, index, id, -5), "-", "Decrease starting " + id.ToLowerInvariant() + ".", canDecrease, false, "-", value.ToString(CultureInfo.InvariantCulture), canDecrease ? null : "Conditions are limited to 0-100."),
                    IncreaseAction = Action(CharacterEditorCommand.Step(CharacterEditorCommandKind.StepCondition, scope, index, id, 5), "+", "Increase starting " + id.ToLowerInvariant() + ".", canIncrease, false, "+", value.ToString(CultureInfo.InvariantCulture), canIncrease ? null : "Conditions are limited to 0-100."),
                    TextAction = Action(CharacterEditorCommand.ValueCommand(CharacterEditorCommandKind.SetCondition, scope, index, id, value.ToString(CultureInfo.InvariantCulture)), value.ToString(CultureInfo.InvariantCulture), "Enter a starting " + id.ToLowerInvariant() + " value from 0 to 100.", true, false, "TX", value.ToString(CultureInfo.InvariantCulture))
                };
            }
            return rows;
        }

        private static ScenarioAuthoringInspectorSection BuildSurvivorModFieldsSection(FamilyMemberConfig member, CharacterMemberScope scope, int index)
        {
            ...
            {
                ScenarioSurvivorModFieldRowViewModel row = BuildSurvivorModFieldRow(member, fields[i], scope, index);
                if (row != null)
                    rows.Add(row);
            }
            ...
            if (HasMissingModFieldNotice(rows))
                items.Add(ScenarioInspectorItemFactory.ActionItem(ScenarioInspectorItemFactory.Action(
                    ...
                    "Resolve Mod Gating",
                    "Open guidance for missing actor-authoring providers.",
                    true,
                    false,
                    "MOD")));
            return new ScenarioAuthoringInspectorSection
        ...
        private static ScenarioSurvivorModFieldRowViewModel BuildSurvivorModFieldRow(FamilyMemberConfig member, ActorAuthoringFieldDefinition field, CharacterMemberScope scope, int index)
        {
            if (field == null)
                return null;
            ...
            string fieldKey = ScenarioActorAuthoringFieldStore.BuildFieldToken(field);
            ...
            if (field.ValueType == ActorAuthoringFieldValueType.Bool)
            {
                ...
                return new ScenarioSurvivorModFieldRowViewModel { Kind = ScenarioSurvivorModFieldControlKind.Toggle, Label = field.Label, ValueText = enabled ? "On" : "Off", HelpText = help, ToggleAction = Action(CharacterEditorCommand.ValueCommand(CharacterEditorCommandKind.ToggleField, scope, index, fieldKey, null), enabled ? "On" : "Off", help, true, enabled, enabled ? "ON" : "OFF", null) };
            }
            if (field.ValueType == ActorAuthoringFieldValueType.Int || field.ValueType == ActorAuthoringFieldValueType.Float)
                return new ScenarioSurvivorModFieldRowViewModel { Kind = ScenarioSurvivorModFieldControlKind.Stepper, Label = field.Label, ValueText = value, HelpText = help, DecreaseAction = Action(CharacterEditorCommand.Step(CharacterEditorCommandKind.StepField, scope, index, fieldKey, -1), "-", "Decrease " + field.Label + ".", true, false, "-", value), IncreaseAction = Action(CharacterEditorCommand.Step(CharacterEditorCommandKind.StepField, scope, index, fieldKey, 1), "+", "Increase " + field.Label + ".", true, false, "+", value) };
            if (field.ValueType == ActorAuthoringFieldValueType.String)
                return new ScenarioSurvivorModFieldRowViewModel { Kind = ScenarioSurvivorModFieldControlKind.Text, Label = field.Label, ValueText = value, HelpText = help, TextAction = Action(CharacterEditorCommand.ValueCommand(CharacterEditorCommandKind.SetFieldText, scope, index, fieldKey, value), field.Label, help, true, false, "TXT", null) };
            if (field.ValueType == ActorAuthoringFieldValueType.StringEnum)
                return new ScenarioSurvivorModFieldRowViewModel { Kind = ScenarioSurvivorModFieldControlKind.Enum, Label = field.Label, ValueText = value, HelpText = help, CycleAction = Action(CharacterEditorCommand.ValueCommand(CharacterEditorCommandKind.CycleFieldEnum, scope, index, fieldKey, null), value, help, true, false, "EN", null) };
            if (field.ValueType == ActorAuthoringFieldValueType.Color)
            {
                Color color;
                if (!ScenarioEditorCharacterAppearanceService.TryParseColorHex(value, out color))
                    color = Color.white;
                return new ScenarioSurvivorModFieldRowViewModel
                {
                    ...
                    {
                        Channel = "mod:" + ScenarioAutomationIdCodec.EncodeToken(fieldKey),
                        Label = field.Label,
                        Hex = value,
                        Color = color,
                        OpenColorPickerCommand = CharacterEditorCommand.ValueCommand(CharacterEditorCommandKind.OpenFieldColorPicker, scope, index, fieldKey, null),
                        ApplyColorCommand = CharacterEditorCommand.ValueCommand(CharacterEditorCommandKind.SetFieldColor, scope, index, fieldKey, value)
                    }
                };
            }
            return null;
        ...
        {
            return Mathf.Clamp(value, ShelteredScenarioEditor.Domain.People.ScenarioFamilyConfigurationPolicy.StatMin, ShelteredScenarioEditor.Domain.People.ScenarioFamilyConfigurationPolicy.StatMax);
        }
        ...
        private static string FormatTexture(FamilyMemberAppearanceConfig appearance, ScenarioEditorCharacterTexturePart part)
        {
            if (appearance == null)
                return "default";
            if (part == ScenarioEditorCharacterTexturePart.Head) return !string.IsNullOrEmpty(appearance.HeadTextureId) ? appearance.HeadTextureId : (!string.IsNullOrEmpty(appearance.HeadTexturePath) ? appearance.HeadTexturePath : "default");
            if (part == ScenarioEditorCharacterTexturePart.Torso) return !string.IsNullOrEmpty(appearance.TorsoTextureId) ? appearance.TorsoTextureId : (!string.IsNullOrEmpty(appearance.TorsoTexturePath) ? appearance.TorsoTexturePath : "default");
        ...
        private static ScenarioAuthoringInspectorAction Action(
            ...
            string disabledReason = null)
        {
            ScenarioAuthoringInspectorAction action = ScenarioInspectorItemFactory.Action(command, label, hint, enabled, emphasized, icon, detail);
