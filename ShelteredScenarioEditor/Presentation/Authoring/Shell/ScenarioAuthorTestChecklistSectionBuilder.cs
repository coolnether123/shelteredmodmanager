...
namespace ShelteredScenarioEditor.Presentation.Authoring.Shell
{
    internal sealed class ScenarioAuthorTestChecklistSectionBuilder
    {
        ...
        internal ScenarioAuthoringInspectorSection BuildTestSection(ScenarioEditorSession session)
        {
            ...
            for (int i = 0; i < steps.Length; i++)
            {
                ...
                ScenarioAuthorTestChecklistItem entry = _service.FindItem(session, step.Id);
                ...
                items.Add(Item.ActionItem(Item.Action(
                    ShellUxCommand.ChecklistToggle(step.Id),
                ...
                note.Action = Item.Action(
                    ShellUxCommand.ChecklistNote(step.Id, string.Empty),
                    "Save note",
                    note.HoverHint,
                    true,
                    false,
        ...
        internal ScenarioAuthoringInspectorItem BuildExportSummary(ScenarioEditorSession session)
        {
            int complete = _service.CountChecked(session);
