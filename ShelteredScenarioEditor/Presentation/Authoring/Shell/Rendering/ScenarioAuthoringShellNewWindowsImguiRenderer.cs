...
namespace ShelteredScenarioEditor.Presentation.Authoring.Shell
{
    ...
    internal sealed partial class ScenarioAuthoringShellImguiRenderModule
    {
        ...
        private void DrawMetadataWarning(Rect rect, string warning)
        {
            ...
            style.normal.textColor = _uiContext.Styles.Theme.Palette.SemanticWarningStrong;
        ...
        private static string MetadataWarning(ScenarioAuthoringInspectorItem item)
        {
            ...
            if (string.Equals(label, "Author", StringComparison.OrdinalIgnoreCase)
                && (value.Length == 0 || string.Equals(value, ShelteredAPI.Scenarios.Public.ShelteredScenarioAuthoring.DefaultAuthor, StringComparison.OrdinalIgnoreCase)))
                return "Replace the placeholder author";
            if (string.Equals(label, "Version", StringComparison.OrdinalIgnoreCase)
                && (value.Length == 0 || string.Equals(value, ShelteredAPI.Scenarios.Public.ShelteredScenarioAuthoring.DefaultVersion, StringComparison.OrdinalIgnoreCase)))
        ...
        {
            _newWindowsAssetFilter = _rendererInteraction.AssetInventoryFilter;
            GUILayout.BeginVertical(_uiContext.Styles.Section);
            ...
            for (int i = 0; section.Items != null && i < section.Items.Length; i++)
            {
                ...
                if (DrawPlainButton(rect, new GUIContent(label), active ? _activeButtonStyle : _buttonStyle, true))
                {
                    ExecuteRendererCommand(RendererInteractionCommand.ForKey(
                        RendererInteractionCommandKind.SelectAssetInventoryFilter,
                        RendererInteractionAutomationIds.ActionRendererAssetInventoryFilterPrefix,
                        filter));
                    _newWindowsAssetFilter = _rendererInteraction.AssetInventoryFilter;
                    Event.current.Use();
                }
