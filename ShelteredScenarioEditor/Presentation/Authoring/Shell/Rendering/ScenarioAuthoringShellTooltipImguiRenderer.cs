...
namespace ShelteredScenarioEditor.Presentation.Authoring.Shell{
    internal sealed partial class ScenarioAuthoringShellImguiRenderModule
    {
        ...
        private RichHoverHelpModel BuildTopicRichHoverHelpModel(string topicId)
        {
            ...
            actions.Add(RichHoverAction(ShellUxCommand.HelpTopic(page.Id), "Open Topic", "Open the full workshop help topic.", true, true));
            if (page.Stage != ScenarioStageKind.None)
                actions.Add(RichHoverAction(ShellUxCommand.SelectStage(page.Stage), "Go", "Open the related workspace.", true, false));
            else if (!string.IsNullOrEmpty(page.WindowId))
                actions.Add(RichHoverAction(ShellUxCommand.ToggleWindow(page.WindowId), "Focus", "Focus the related editor panel.", true, false));

            if (!string.IsNullOrEmpty(page.TourId))
                actions.Add(RichHoverAction(ShellUxCommand.Tour(page.TourId), "Tour", "Start the related spotlight tour.", true, false));
        ...
        private static ScenarioAuthoringInspectorAction RichHoverAction(ScenarioAuthoringCommand command, string label, string hint, bool enabled, bool emphasized)
        {
            ScenarioAuthoringInspectorAction action = RichHoverAction(command != null ? command.AutomationId : string.Empty, label, hint, enabled, emphasized);
