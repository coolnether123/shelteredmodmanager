...
namespace ShelteredScenarioEditor.Presentation.Authoring.Shell{
    internal sealed partial class ScenarioAuthoringShellImguiRenderModule
    {
            ...
            ScenarioAuthoringInputCaptureService inputCapture)
        {
            ...
            if (hasTarget)
                HandleSpotlightTargetClick(tutorial.TargetCommand, targetRect, true);
            ...
            ScenarioAuthoringInputCaptureService inputCapture)
        {
            ...
            if (hasTarget)
                HandleSpotlightTargetClick(tour.TargetCommand, targetRect, false);
            inputCapture.RegisterInteractiveRect(availableRect);
            inputCapture.RegisterInteractiveRect(calloutRect);
            if (hasTarget)
                inputCapture.RegisterInteractiveRect(targetRect);
            ...
            ScenarioAuthoringShellViewModel shell)
        {
            ...
            Rect registered;
            if (_tourTargets != null && _tourTargets.TryGet(targetId, out registered))
                return registered;
        ...
        private void HandleSpotlightTargetClick(ScenarioAuthoringCommand targetCommand, Rect targetRect, bool tutorial)
        {
            ...
            if (targetCommand != null)
                _backend.ExecuteCommand(targetCommand);

            string automationId = tutorial ? ScenarioAuthoringActionIds.ActionTutorialNext : ScenarioAuthoringActionIds.ActionTourNext;
            _backend.ExecuteCommand(ShellUxCommand.Simple(
                tutorial ? ShellUxCommandKind.TutorialNext : ShellUxCommandKind.TourNext,
                automationId));
            evt.Use();
        }
