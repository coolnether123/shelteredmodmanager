...
namespace ShelteredScenarioEditor.Presentation.Authoring.Shell{
    ...
    internal sealed partial class ScenarioAuthoringShellImguiRenderModule
    {
        ...
        private void ResetGlobalSearchIndex()
        {
            ...
            _rendererInteraction.GlobalSearchQuery = string.Empty;
            _globalSearchFocused = false;
            _globalSearchNeedsFocus = false;
            _globalSearchSelectedIndex = 0;
        }

        private ScenarioDefinition ResolveWorkingDefinition()
        {
            ScenarioEditorSession session = _sessionStore != null ? _sessionStore.Current : null;
            return session != null ? session.WorkingDefinition : null;
        }

        private List<string> ResolveNamedVersions()
        {
            List<string> names = new List<string>();
            ScenarioDraftSnapshotInfo[] all = _snapshots != null ? _snapshots.ListSnapshots() : null;
        ...
        {
            _globalSearchText = _rendererInteraction.GlobalSearchQuery;
            ...
            if (!string.Equals(typed, _globalSearchText, StringComparison.Ordinal))
            {
                ExecuteRendererCommand(RendererInteractionCommand.ForValue(
                    RendererInteractionCommandKind.SetGlobalSearchQuery,
                    RendererInteractionAutomationIds.ActionRendererGlobalSearchQueryPrefix,
                    typed));
                _globalSearchText = _rendererInteraction.GlobalSearchQuery;
                _globalSearchSelectedIndex = 0;
            }
        ...
        private void ActivateGlobalSearchEntry(ScenarioGlobalSearchEntry entry)
        {
            ...
            _backend.ExecuteCommand(entry.Command);
        ...
        {
            _backend.ExecuteCommand(ShellUxCommand.Simple(ShellUxCommandKind.CloseGlobalSearch, ScenarioAuthoringActionIds.ActionShellCloseGlobalSearch));
        ...
        private Rect DrawTopBarGlobalSearchButton(Rect windowMenuButtonRect, Rect animatedRect, bool compact)
        {
            ...
            {
                _backend.ExecuteCommand(ShellUxCommand.Simple(ShellUxCommandKind.ToggleGlobalSearch, ScenarioAuthoringActionIds.ActionShellToggleGlobalSearch));
                if (Event.current != null)
                    Event.current.Use();
            }
        ...
        private void DrawSearchPlaceholder(Rect rect, string value, string placeholder)
        {
            ...
            Color ink = _uiContext.Styles.Theme.Palette.TextPrimary;
