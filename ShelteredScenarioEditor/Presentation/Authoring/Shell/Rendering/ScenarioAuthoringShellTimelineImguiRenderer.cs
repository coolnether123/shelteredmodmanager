...
namespace ShelteredScenarioEditor.Presentation.Authoring.Shell{
    internal sealed partial class ScenarioAuthoringShellImguiRenderModule
    {
        ...
        private void DrawTimelineChip(Rect rect, TimelineChipInfo chip)
        {
            ...
            {
                ExecuteInspectorAction(chip.Action);
        ...
        private static bool IsTimelineAddAction(ScenarioAuthoringInspectorAction action)
        {
            if (action == null || string.IsNullOrEmpty(action.Id))
                return false;
            GameplayScheduleCommand gameplay = action.Command as GameplayScheduleCommand;
            if (gameplay != null && (gameplay.Kind == GameplayScheduleCommandKind.AddWeather || gameplay.Kind == GameplayScheduleCommandKind.AddFutureSurvivor))
                return true;
            return string.Equals(action.Id, ScenarioAuthoringActionIds.ActionScheduledActionAdd, StringComparison.Ordinal)
