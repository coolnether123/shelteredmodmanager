...
namespace ShelteredScenarioEditor.Presentation.Authoring.Shell{
    internal sealed partial class ScenarioAuthoringShellImguiRenderModule
    {
        ...
        private void DrawPlacementHudCore(Rect rect, ScenarioAuthoringShellWindowViewModel[] windows)
        {
            ...
            ShellUxCommand snapCommand = ShellUxCommand.SettingToggle("visuals.snap_to_grid");
            string snapActionId = snapCommand.AutomationId;
            ...
            if (DrawPlainButton(
                ...
                true))
            {
                _backend.ExecuteCommand(snapCommand);
            ...
            ShellUxCommand gridCommand = ShellUxCommand.SettingToggle("visuals.show_grid");
            string gridActionId = gridCommand.AutomationId;
            ...
            if (DrawPlainButton(
                ...
                true))
            {
                _backend.ExecuteCommand(gridCommand);
                if (Event.current != null)
                    Event.current.Use();
            }
            x += actionWidth + actionGap;
