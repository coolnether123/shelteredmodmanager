        ...
        private void DrawHelpShortcutsBody(ScenarioAuthoringShortcutOverlayViewModel shortcuts, float availableWidth)
        {
            ...
            for (int column = 0; column < columnCount; column++)
            {
                ...
                for (int g = start; g < end; g++)
                {
                    ...
                    bool expanded = _rendererInteraction.GetDisclosureExpanded(key, roomy && group.IsActiveContext);
                    ...
                    {
                        ExecuteRendererCommand(RendererInteractionCommand.ForKey(RendererInteractionCommandKind.ToggleDisclosure, RendererInteractionAutomationIds.ActionRendererWorkshopGroupTogglePrefix, key));
                        if (Event.current != null) Event.current.Use();
                        expanded = _rendererInteraction.GetDisclosureExpanded(key, roomy && group.IsActiveContext);
                    }
                    if (expanded)
                    {
