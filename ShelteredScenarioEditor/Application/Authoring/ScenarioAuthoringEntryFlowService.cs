...
using ShelteredScenarioEditor.Application.Authoring.Templates;
...
namespace ShelteredScenarioEditor.Application.Authoring{
    ...
    {
        private const string RuntimeObjectName = "ShelteredScenarioEditor.EntryFlow";
        ...
        private ScenarioAuthoringSelectionOutcome _selectionOutcome;
        ...
        private void HandleSessionTransition(ScenarioAuthoringSessionTransition transition)
        {
            ...
            {
                SetLoadingStatus("Status: " + (transition.Status ?? "waiting for the current mode load."));
                return;
            }
            if (transition.Kind != ScenarioAuthoringSessionTransitionKind.PhaseChanged)
                return;
            if (transition.Phase == ScenarioAuthoringSessionPhase.ReloadPending)
                BeginReload(transition.Pending, transition.Reason);
            else if (transition.Phase == ScenarioAuthoringSessionPhase.Inactive)
                Hide("Authoring session closed.");
        }
        ...
        public void MarkEditorReady(ScenarioAuthoringSession session, bool worldFullyReady)
        {
            ...
            _worldReady = true;
            if (_selectionOutcome == ScenarioAuthoringSelectionOutcome.Pending)
                _selectionOutcome = ScenarioAuthoringSelectionOutcome.Succeeded;
        ...
        public void SelectWizardBase(string baseKey)
        {
            ...
            if (string.Equals(_selectedBaseKey, baseKey, StringComparison.Ordinal)
                ...
                && _selectionOutcome != ScenarioAuthoringSelectionOutcome.Failed)
                return;

            _selectedBaseKey = baseKey;
            _selectionToken++;
            _selectionOutcome = ScenarioAuthoringSelectionOutcome.Pending;
        ...
        public void CancelWizard()
        {
            ...
            string message;
            if (_sessionLifecycle.CancelUncommittedWizardDraft(draftId, "Canceled the scenario setup wizard.", out message))
                return;

            if (_sessionLifecycle.RequestCloseToMainMenu(null, "Canceled the scenario setup wizard.", out message))
            {
                SetSnapshot(CreateLoadingSnapshot(
        ...
        private void ApplyBaseMode(ScenarioEditorSession editorSession, ScenarioBaseGameMode mode)
        {
            ...
            if (_baseModeReloadService == null
                || !_baseModeReloadService.SaveAndReload(editorSession, mode, ShelteredAPI.Scenarios.Definitions.ScenarioBaseFamilyChoices.KeepCurrentCast, true, out message))
            {
        ...
        private void ApplyCustomScenario(ScenarioEditorSession editorSession, string scenarioId)
        {
            ...
            ScenarioDefinition copy = ShelteredScenarioEditor.Application.Runtime.ScenarioEditorDefinitionCloner.Clone(source);
            if (copy == null)
            {
                _dispatchedToken = _selectionToken;
                _selectionOutcome = ScenarioAuthoringSelectionOutcome.Failed;
        ...
        {
            ScenarioInfo[] entries = new ScenarioInfo[0];
            try
            {
                entries = _definitionCatalog != null
                    ? _definitionCatalog.ListDefinitions()
                    : new ScenarioInfo[0];
            }
            catch (Exception ex)
            {
            ...
            {
                ScenarioInfo entry = entries[i];
                if (entry == null || string.IsNullOrEmpty(entry.Id))
                    continue;

                ScenarioDefinition editableDefinition;
                bool hasEditableDefinition = TryLoadEditableDefinition(entry.Id, out editableDefinition);
                ScenarioValidationResult validation = _definitionCatalog.ValidateDefinition(entry.Id);
                bool enabled = hasEditableDefinition && validation != null && validation.IsValid;
                string disabledReason = null;
                if (validation == null || !validation.IsValid)
                    disabledReason = "The installed scenario does not currently pass validation.";
                ...
                string detail = editableDefinition != null && !string.IsNullOrEmpty(editableDefinition.Description)
                    ...
                    : "No scenario description was provided.";
                string author = editableDefinition != null && !string.IsNullOrEmpty(editableDefinition.Author)
                    ? editableDefinition.Author
                    : entry.OwnerModId;
                string key = BaseKeyCustomPrefix + entry.Id;
                cards.Add(new ScenarioAuthoringEntryBaselineCard
                {
                    Key = key,
                    Title = Safe(entry.DisplayName, entry.Id),
                    Badge = "COPY",
        ...
        private sealed class ScenarioAuthoringEntryFlowRuntime : MonoBehaviour
        {
            ...
            private void EnsureLoadingCanvas()
            {
                ...
                GameObject root = new GameObject("ShelteredScenarioEditor.EntryFlow.LoadingCanvas");
    ...
    internal enum ScenarioAuthoringSelectionOutcome
    {
        Pending,
        Succeeded,
        Failed
