...
namespace ShelteredScenarioEditor.Application.Authoring
{
    ...
    internal sealed class ScenarioSurvivorAuthoringOperations
    {
        ...
        public ScenarioSurvivorAuthoringOperations(ScenarioEditorCharacterAppearanceService appearanceService)
        {
        ...
        public void RandomizeDeclaredFields(FamilyMemberConfig member)
        {
            ...
            ShelteredScenarioEditor.Domain.People.ScenarioFamilyConfigurationPolicy.EnsureCoreStats(member);
            for (int i = 0; member.Stats != null && i < member.Stats.Count; i++)
            {
                StatOverride stat = member.Stats[i];
                if (stat != null)
                    stat.Value = UnityEngine.Random.Range(ShelteredScenarioEditor.Domain.People.ScenarioFamilyConfigurationPolicy.StatMin, ShelteredScenarioEditor.Domain.People.ScenarioFamilyConfigurationPolicy.StatMax + 1);
            }
        ...
        public void RandomizeAppearance(FamilyMemberConfig member)
        {
            ...
            {
                member.Appearance.HeadTextureId = _appearanceService.RandomTextureId(member.Appearance.MeshId, ScenarioEditorCharacterTexturePart.Head);
                member.Appearance.TorsoTextureId = _appearanceService.RandomTextureId(member.Appearance.MeshId, ScenarioEditorCharacterTexturePart.Torso);
                member.Appearance.LegTextureId = _appearanceService.RandomTextureId(member.Appearance.MeshId, ScenarioEditorCharacterTexturePart.Legs);
                member.Appearance.HairColorHex = _appearanceService.RandomColorHex(ScenarioEditorCharacterColorPart.Hair);
                member.Appearance.SkinColorHex = _appearanceService.RandomColorHex(ScenarioEditorCharacterColorPart.Skin);
                member.Appearance.ShirtColorHex = _appearanceService.RandomColorHex(ScenarioEditorCharacterColorPart.Shirt);
                member.Appearance.PantsColorHex = _appearanceService.RandomColorHex(ScenarioEditorCharacterColorPart.Pants);
            }
        ...
        public static FutureSurvivorDefinition CloneFutureSurvivor(FutureSurvivorDefinition source)
        {
            ...
            {
                Id = source.Id,
                ActorRef = ScenarioCastMemberReferenceCatalog.CopyActorRef(ShelteredAPI.Scenarios.Public.ShelteredScenarioAuthoring.ResolveFutureSurvivorActorReference(source)),
                Arrival = CopySchedule(source.Arrival),
                AskToJoin = source.AskToJoin,
                Survivor = CloneMember(source.Survivor)
            };
            CopyActorComponents(source.ActorComponents, copy.ActorComponents);
            if (copy.Survivor != null)
            {
                copy.Survivor.ActorRef = null;
                copy.Survivor.ActorComponents.Clear();
            }
            return copy;
        }
