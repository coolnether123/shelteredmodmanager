...
namespace ShelteredScenarioEditor.Application.Authoring.Tutorial{
    internal sealed class ScenarioAuthoringTutorialService
    {
        ...
        public bool HandleAction(ScenarioAuthoringState state, ShellUxCommandKind kind, out string message)
        {
            message = null;
            if (state == null)
                return false;
            ...
            switch (kind)
            {
                case ShellUxCommandKind.TutorialNext: return Advance(state, true, out message);
                case ShellUxCommandKind.TutorialBack: return Back(state, out message);
                case ShellUxCommandKind.TutorialSkip: return Skip(state, out message);
                case ShellUxCommandKind.TutorialSkipPrompt: return ShowSkipPrompt(out message);
                case ShellUxCommandKind.TutorialSkipCancel: return HideSkipPrompt(out message);
                case ShellUxCommandKind.TutorialReset: return Reset(state, out message);
                case ShellUxCommandKind.HelpPageNext: return StepHelpPage(state, 1, out message);
                case ShellUxCommandKind.HelpPagePrevious: return StepHelpPage(state, -1, out message);
        ...
        private bool CompleteTour(ScenarioAuthoringState state, out string message)
        {
            ...
            ScenarioEditorState editorState = _editorStateSession != null
                ? _editorStateSession.Current
                : null;
            if (editorState != null)
            {
                editorState.AddCompletedTour(tour.Id);
                _editorStateSession.SaveCurrent();
            }
        ...
        private static void ApplyTourOpenAction(
            ...
            ScenarioAuthoringLayoutService layoutService)
        {
            if (state == null || step == null || layoutService == null)
                return;

            ShellUxCommand command = step.OpenCommand as ShellUxCommand;
            if (command != null && command.Kind == ShellUxCommandKind.SelectStage)
            {
                layoutService.SelectStage(state, command.Stage);
                return;
            }

            if (command != null && command.Kind == ShellUxCommandKind.OpenHelpTopic)
            {
                ApplyHelpNavigation(state, TutorialContent.FindHelpPage(command.Key), layoutService);
                return;
            }

            if (command != null && command.Kind == ShellUxCommandKind.ToggleWindow)
            {
                layoutService.SetWindowOpen(state, command.Key, true);
                return;
            }

            if (step.OpenTool.HasValue)
                layoutService.SelectTool(state, step.OpenTool.Value);
        }
        ...
        private void EnsureActivation(ScenarioAuthoringState state, TutorialStep step, bool satisfied)
        {
            ...
            string draftId = session != null && session.WorkingDefinition != null
