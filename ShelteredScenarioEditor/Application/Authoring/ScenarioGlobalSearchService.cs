...
namespace ShelteredScenarioEditor.Application.Authoring{
    ...
    /// <summary>
    /// One ranked, activatable entry in the creator command palette. Stable IDs are
    /// selectors; the command carries the executable route.
    /// </summary>
    internal sealed class ScenarioGlobalSearchEntry
    {
            ...
            ScenarioAuthoringCommand command)
        {
            ...
            Enabled = enabled;
            Command = command;
            TargetActionIds = new string[0];
            ActionIds = command != null && !string.IsNullOrEmpty(command.AutomationId)
                ? new[] { command.AutomationId }
                : new string[0];
        }
        ...
        public ScenarioGlobalSearchEntry(
            ...
            string context,
            bool enabled,
            ScenarioGlobalSearchRouteStep[] route)
        {
            Kind = kind;
            KindLabel = kindLabel ?? string.Empty;
            Name = string.IsNullOrEmpty(name) ? "(unnamed)" : name;
            Context = context ?? string.Empty;
            Enabled = enabled;
            Command = route != null && route.Length > 0 ? new ScenarioGlobalSearchRouteCommand(route) : null;
            TargetActionIds = RouteIds(route);
            ActionIds = Command != null && !string.IsNullOrEmpty(Command.AutomationId)
                ? new[] { Command.AutomationId }
                : new string[0];
        }
        ...
        private static string[] RouteIds(ScenarioGlobalSearchRouteStep[] route)
        {
            List<string> ids = new List<string>();
            for (int i = 0; route != null && i < route.Length; i++)
            {
                string id = route[i] != null ? route[i].AutomationId : null;
    ...
    internal static class ScenarioGlobalSearchService
    {
            ...
            ScenarioDraftSnapshotService snapshots)
        {
            ...
            return Rank(
                ...
                rendererInteraction.GlobalSearchQuery,
                MaxResults);
        }
        ...
        private static void CollectWorkspaceEntries(ScenarioAuthoringShellViewModel shell, List<ScenarioGlobalSearchEntry> entries)
        {
            ...
            for (int windowIndex = 0; shell != null && shell.Windows != null && windowIndex < shell.Windows.Length; windowIndex++)
            {
                ...
                ShellUxCommand openCommand = WorkspaceOpenCommand(workspace.Id);
                for (int i = 0; workspace.Subtabs != null && i < workspace.Subtabs.Length; i++)
                {
                    ...
                    if (subtab == null || subtab.SelectAction == null || string.IsNullOrEmpty(subtab.Label))
                        continue;
                    AddWorkspaceEntry(entries, seen, ScenarioGlobalSearchKind.WorkspaceSubtab, "Workspace", subtab.Label, window.Title, openCommand, subtab.SelectAction);
                }
                ...
                for (int i = 0; navigator != null && navigator.Groups != null && i < navigator.Groups.Length; i++)
                {
                    ...
                    CollectWorkspaceRows(entries, seen, group != null ? group.Rows : null, window.Title, group != null ? group.Label : null, openCommand);
                }
            }
        }
            ...
            ShellUxCommand openCommand)
        {
            for (int i = 0; rows != null && i < rows.Length; i++)
            {
                ...
                if (row.SelectAction != null && !string.IsNullOrEmpty(row.Title))
                    AddWorkspaceEntry(entries, seen, ScenarioGlobalSearchKind.NavigatorEntity, "Navigator", row.Title, (workspaceLabel ?? "Workspace") + " / " + (groupLabel ?? "Items"), openCommand, row.SelectAction);
                CollectWorkspaceRows(entries, seen, row.Children, workspaceLabel, row.Title, openCommand);
            }
        }
        ...
        private static void AddWorkspaceEntry(
            ...
            string context,
            ShellUxCommand openCommand,
            ScenarioAuthoringInspectorAction selectAction)
        {
            ScenarioAuthoringCommand selectCommand = selectAction != null ? selectAction.Command : null;
            string selectAutomationId = selectCommand != null ? selectCommand.AutomationId : string.Empty;
            string key = (openCommand != null ? openCommand.AutomationId : string.Empty) + "\n" + selectAutomationId;
            if (selectCommand == null || string.IsNullOrEmpty(selectAutomationId) || !seen.Add(key))
                return;
            entries.Add(new ScenarioGlobalSearchEntry(
                ...
                true,
                openCommand == null
                    ? new[] { ScenarioGlobalSearchRouteStep.Typed(selectCommand) }
                    : new[] { ScenarioGlobalSearchRouteStep.Typed(openCommand), ScenarioGlobalSearchRouteStep.Typed(selectCommand) }));
        }

        private static ShellUxCommand WorkspaceOpenCommand(string workspaceId)
        {
            if (string.Equals(workspaceId, ScenarioStoryFocusedEditorActions.WorkspaceId, StringComparison.Ordinal))
                return ShellUxCommand.SelectStage(ScenarioStageKind.Quests);
            if (string.Equals(workspaceId, ScenarioCastWorkspaceActions.WorkspaceId, StringComparison.Ordinal))
                return ShellUxCommand.SelectStage(ScenarioStageKind.People);
            if (string.Equals(workspaceId, ScenarioSuppliesWorkspaceActions.WorkspaceId, StringComparison.Ordinal))
                return ShellUxCommand.SelectStage(ScenarioStageKind.InventoryStorage);
            if (string.Equals(workspaceId, ScenarioMapWorkspaceSelection.WorkspaceId, StringComparison.Ordinal))
                return ShellUxCommand.SelectStage(ScenarioStageKind.Map);
            return null;
        }
        ...
        private static void CollectStoryStages(ScenarioDefinition definition, List<ScenarioGlobalSearchEntry> entries)
        {
            ...
            ShellUxCommand storyTab = ShellUxCommand.SelectStage(ScenarioStageKind.Quests);
            for (int i = 0; i < flow.Stages.Count; i++)
            {
                ...
                entries.Add(new ScenarioGlobalSearchEntry(
                    ...
                    true,
                    Route(storyTab, WorkspaceSelect(ScenarioStoryFocusedEditorActions.WorkspaceId, ScenarioStoryFocusedEditorActions.FlowSubtabId, ScenarioStoryFocusedEditorActions.StageEntityId(definition, i)))));
                ...
                for (int s = 0; s < stepCount; s++)
                {
                    ...
                    entries.Add(new ScenarioGlobalSearchEntry(
                        ...
                        true,
                        Route(storyTab, WorkspaceSelect(ScenarioStoryFocusedEditorActions.WorkspaceId, ScenarioStoryFocusedEditorActions.FlowSubtabId, ScenarioStoryFocusedEditorActions.SceneEntityId(definition, i, s)))));
                }
            }
        }
        ...
        private static void CollectStoryCharacters(ScenarioDefinition definition, List<ScenarioGlobalSearchEntry> entries)
        {
            ...
            for (int i = 0; i < characters.Count; i++)
            {
                ...
                entries.Add(new ScenarioGlobalSearchEntry(
                    ...
                    true,
                    Route(storyTab, WorkspaceSelect(ScenarioStoryFocusedEditorActions.WorkspaceId, ScenarioStoryFocusedEditorActions.CharactersSubtabId, ScenarioStoryFocusedEditorActions.CharacterEntityId(definition, i)))));
            }
        }
        ...
        private static void CollectConversations(ScenarioDefinition definition, List<ScenarioGlobalSearchEntry> entries)
        {
            ...
            for (int i = 0; i < conversations.Conversations.Count; i++)
            {
                ...
                entries.Add(new ScenarioGlobalSearchEntry(
                    ...
                    true,
                    Route(storyTab, WorkspaceSelect(ScenarioStoryFocusedEditorActions.WorkspaceId, ScenarioStoryFocusedEditorActions.ConversationsSubtabId, ScenarioStoryFocusedEditorActions.ConversationEntityId(definition, i)))));
            }
        }
        ...
        private static void CollectCast(ScenarioDefinition definition, List<ScenarioGlobalSearchEntry> entries)
        {
            ...
            ShellUxCommand peopleTab = ShellUxCommand.SelectStage(ScenarioStageKind.People);
            if (family.Members != null)
            {
                for (int i = 0; i < family.Members.Count; i++)
                {
                    ...
                    entries.Add(new ScenarioGlobalSearchEntry(
                        ...
                        "Starting survivor",
                        true,
                        Route(peopleTab, WorkspaceSelect(ScenarioCastWorkspaceActions.WorkspaceId, ScenarioCastWorkspaceActions.SubtabId, ScenarioCastWorkspaceActions.StartingEntityId(definition, i)))));
                }
            }
            ...
            if (family.FutureSurvivors != null)
            {
                for (int i = 0; i < family.FutureSurvivors.Count; i++)
                {
                    ...
                    entries.Add(new ScenarioGlobalSearchEntry(
                        ...
                        "Future survivor",
                        true,
                        Route(peopleTab, WorkspaceSelect(ScenarioCastWorkspaceActions.WorkspaceId, ScenarioCastWorkspaceActions.SubtabId, ScenarioCastWorkspaceActions.FutureEntityId(definition, i)))));
                }
            }
        }
        ...
        private static void CollectQuests(ScenarioDefinition definition, List<ScenarioGlobalSearchEntry> entries)
        {
            ...
            for (int i = 0; i < definition.Quests.Quests.Count; i++)
            {
                ...
                entries.Add(new ScenarioGlobalSearchEntry(
                    ...
                    true,
                    Route(storyTab, WorkspaceSelect(ScenarioStoryFocusedEditorActions.WorkspaceId, ScenarioStoryFocusedEditorActions.QuestPopupsSubtabId, ScenarioStoryFocusedEditorActions.QuestEntityId(definition, i)))));
            }
        }

        private static void CollectTimeline(ScenarioDefinition definition, List<ScenarioGlobalSearchEntry> entries)
        {
            ShellUxCommand eventsTab = ShellUxCommand.SelectStage(ScenarioStageKind.Events);
        ...
        private static void CollectMapLocations(ScenarioDefinition definition, List<ScenarioGlobalSearchEntry> entries)
        {
            ...
            ShellUxCommand mapTab = ShellUxCommand.SelectStage(ScenarioStageKind.Map);
            for (int i = 0; i < map.Locations.Count; i++)
            {
                ...
                entries.Add(new ScenarioGlobalSearchEntry(
                    ...
                    "Map / Locations",
                    true,
                    Route(mapTab, WorkspaceSelect(ScenarioMapWorkspaceSelection.WorkspaceId, ScenarioMapWorkspaceSelection.MainSubtabId, ScenarioMapWorkspaceSelection.LocationEntityId(map, i)))));
            }
        }

        private static RendererInteractionCommand WorkspaceSelect(string workspaceId, string subtabId, string entityId)
        {
            return RendererInteractionCommand.ForWorkspace(
                RendererInteractionCommandKind.SelectWorkspaceEntity,
                RendererInteractionAutomationIds.ActionRendererWorkspaceEntitySelectPrefix,
                workspaceId,
                subtabId,
                entityId);
        }

        private static ScenarioGlobalSearchRouteStep[] Route(ScenarioAuthoringCommand openCommand, ScenarioAuthoringCommand selectCommand)
        {
            return new[]
            {
                ScenarioGlobalSearchRouteStep.Typed(openCommand),
                ScenarioGlobalSearchRouteStep.Typed(selectCommand)
            };
        }
        ...
        private static void CollectVersions(IList<string> namedVersions, List<ScenarioGlobalSearchEntry> entries)
        {
            ...
            for (int i = 0; i < namedVersions.Count; i++)
            {
                ...
                entries.Add(new ScenarioGlobalSearchEntry(
                    ...
                    true,
                    ScenarioDraftHistoryCommand.Show()));
            }
        }
