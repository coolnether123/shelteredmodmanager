...
namespace ShelteredScenarioEditor.Application.Authoring{
    internal sealed class ScenarioBuildPlacementAuthoringService
    {
        ...
        {
            public BuildPlacementCommand Command;
            public string Label;
            public string Hint;
        ...
        public bool Execute(ScenarioAuthoringState state, BuildPlacementCommand command, out string message)
        {
            message = null;
            if (command == null)
                return false;
            ...
            switch (command.Kind)
            {
                case BuildPlacementCommandKind.Cancel: return CancelActivePlacement("Placement cancelled.", out message);
                case BuildPlacementCommandKind.CommitGrid: return CommitPlacementAtGridCell(command.GridX, command.GridY, out message);
                case BuildPlacementCommandKind.StartObject: return StartObjectPlacement(command.ObjectType, command.Level, out message);
                case BuildPlacementCommandKind.StartRoom: return StartRoomPlacement(out message);
                case BuildPlacementCommandKind.StartLadder: return StartLadderPlacement(out message);
                case BuildPlacementCommandKind.StartLight: return StartRoomLightPlacement(out message);
                case BuildPlacementCommandKind.DeleteObject: return _deletionService.DeleteObject(target, out message);
                case BuildPlacementCommandKind.DeleteRoom: return _deletionService.DeleteRoom(target, out message);
                case BuildPlacementCommandKind.DeleteLadder: return _deletionService.DeleteLadder(target, out message);
                case BuildPlacementCommandKind.DeleteLight: return _deletionService.DeleteLight(target, out message);
                case BuildPlacementCommandKind.ResetWall: return _deletionService.ResetWall(target, out message);
                case BuildPlacementCommandKind.ResetWire: return _deletionService.ResetWire(target, out message);
                case BuildPlacementCommandKind.ApplyWall: return ApplyWall(target, command.CatalogIndex, command.RuntimeSpriteKey, out message);
                case BuildPlacementCommandKind.ApplyWire: return ApplyWire(target, command.CatalogIndex, command.RuntimeSpriteKey, out message);
                default:
                    message = "Build placement command is not supported.";
                    return false;
            }
        }
        ...
        public bool Update(ScenarioAuthoringState state, ScenarioEditorSession session, out string message)
        {
            ...
            if ((_inputCapture == null || !_inputCapture.KeyboardShortcutHandled)
            ...
            bool suppressWorldInput = _inputCapture != null && _inputCapture.ShouldSuppressWorldInputNow();
        ...
        private bool CommitPlacementAtGridCell(int gridX, int gridY, out string message)
        {
            message = null;
            ...
            UpdateGhostPosition(ShelteredScenarioRuntime.GetShelterGridCellCenter(gridX, gridY));
            ...
            if (_activePlacement.Kind == PlacementSessionKind.Room)
            {
                ...
                return CompleteRoomPlacement(_editorService != null ? _editorService.CurrentSession : null, _activePlacement.Ghost, out message);
            }
            ...
            if (_activePlacement.Kind == PlacementSessionKind.Object)
            {
                ...
                return CompleteObjectPlacement(
                    _editorService != null ? _editorService.CurrentSession : null,
                    _activePlacement.Ghost as Obj_CraftingGhost,
                    out message);
            }

            return TryCompletePlacement(_editorService != null ? _editorService.CurrentSession : null, out message);
        }
        ...
        private bool CompleteRoomPlacement(ScenarioEditorSession session, Obj_GhostBase ghost, out string message)
        {
            ...
            if (!_objectPlacementService.UpsertPlacement(_structurePlacementService.CreateRoomPlacement(
                    gridX,
                    gridY,
                    ShelteredScenarioRuntime.GetShelterGridCellCenter(gridX, gridY),
        ...
        private bool ApplyWall(ScenarioAuthoringTarget target, int wallIndex, string runtimeSpriteKey, out string message)
        {
            ...
            RecordBunkerUndo(_editorService != null ? _editorService.CurrentSession : null, "Apply wall at " + gridX + "," + gridY);
        ...
        private bool ApplyWire(ScenarioAuthoringTarget target, int wireIndex, string runtimeSpriteKey, out string message)
        {
            ...
            int serializedIndex = entry.NativeIndex >= 0 ? appliedIndex : -1;
            RecordBunkerUndo(_editorService != null ? _editorService.CurrentSession : null, "Apply wiring at " + gridX + "," + gridY);
        ...
        private bool IsEditorCameraDragPanning()
        {
            return _editorCamera != null && _editorCamera.ShouldSuppressSelectionClickThisFrame();
        }
        ...
        private static Vector3 ResolveGridPlacementPosition(Vector3 worldPoint)
        {
            int gridX;
            int gridY;
            if (ShelteredScenarioRuntime.TryGetShelterGridCell(worldPoint, out gridX, out gridY))
                return ShelteredScenarioRuntime.GetShelterGridCellCenter(gridX, gridY);
        ...
        private static bool TryResolveActiveGridCell(Vector3 worldPosition, out ShelterRoomGrid grid, out int gridX, out int gridY, out string message)
        {
            ...
            if (ShelteredScenarioRuntime.TryGetShelterGridCell(worldPosition, out gridX, out gridY))
                return true;
        ...
        private static Vector3 ResolveLadderPlacementPosition(Vector3 worldPoint)
        {
            ...
            int gridX;
            int gridY;
            if (grid == null || !ShelteredScenarioRuntime.TryGetShelterGridCell(worldPoint, out gridX, out gridY))
                return worldPoint;

            Vector3 snapped = ShelteredScenarioRuntime.GetShelterGridCellCenter(gridX, gridY);
        ...
        private List<PaletteSectionModel> BuildObjectSections()
        {
            ...
            for (int raw = 0; raw < maxValue; raw++)
            {
                ...
                grouped[sectionTitle].Add(_placementPaletteService.CreateEntry(
                    sectionTitle,
                    BuildPlacementCommand.StartObject(objectType, component.objectLevel > 0 ? component.objectLevel : 1),
        ...
        private PaletteSectionModel BuildStructureSection()
        {
            ...
            {
                Command = BuildPlacementCommand.StartRoom(),
                Label = "Room Tile",
            ...
            entries.Add(new PaletteEntryModel
            {
                Command = BuildPlacementCommand.StartLadder(),
                Label = "Ladder",
            ...
            entries.Add(new PaletteEntryModel
            {
                Command = BuildPlacementCommand.StartLight(),
                Label = "Room Light",
        ...
        private List<PaletteSectionModel> BuildRoomVisualSections(ScenarioAuthoringTarget target)
        {
            ...
            string activeWallKey = ShelteredScenarioRuntime.CreateRuntimeSpriteKey(activeWall);
            ...
            for (int i = 0; wallPalette != null && i < wallPalette.Count; i++)
            {
                ...
                {
                    Command = BuildPlacementCommand.ApplyWall(entry != null ? entry.RuntimeSpriteKey : null),
            ...
            string activeWireKey = ShelteredScenarioRuntime.CreateRuntimeSpriteKey(activeWire);
            ...
            for (int i = 0; wirePalette != null && i < wirePalette.Count; i++)
            {
                ...
                {
                    Command = BuildPlacementCommand.ApplyWire(entry != null ? entry.RuntimeSpriteKey : null),
        ...
        private List<PaletteEntryModel> BuildObjectEntries(IEnumerable<PlacementPaletteService.PaletteEntry> entries, ObjectManager manager)
        {
            ...
            foreach (PlacementPaletteService.PaletteEntry entry in entries)
            {
                ...
                ObjectManager.ObjectType objectType = entry.Command.ObjectType;
                int level = entry.Command.Level;
        ...
        private void RecordBunkerUndo(ScenarioEditorSession session, string description)
        {
            if (session == null || session.WorkingDefinition == null)
                return;

            if (_historyService != null)
                _historyService.RecordBunkerChange(session.WorkingDefinition, description);
        }
    }
}
