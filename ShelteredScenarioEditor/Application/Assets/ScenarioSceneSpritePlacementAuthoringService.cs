...
namespace ShelteredScenarioEditor.Application.Assets{
    internal sealed class ScenarioSceneSpritePlacementAuthoringService
    {
        ...
        public PlacementPickerModel GetPickerModel(ScenarioEditorSession session, ScenarioAuthoringTarget target, string scenarioFilePath, ScenarioAuthoringState state = null)
        {
            ...
            if (HasActivePlacement)
            {
                ...
                model.PlacementSummary = BuildPlacementSummary(_activePlacement.Candidate, _activePlacement.SnapToGridDefault);
            }
            else
            {
                ...
                bool snapToGrid = state == null || state.Settings == null || state.Settings.GetBool("visuals.snap_to_grid", true);
                model.PlacementSummary = activePlacement != null
                    ? "Placement '" + (activePlacement.Id ?? "<placement>") + "' is selected."
                    : "Selecting a sprite starts a " + (snapToGrid ? "snapped" : "free") + " scene sprite placement preview.";
            }
            return model;
        }
        ...
        public bool Execute(ScenarioAuthoringState state, SceneSpritePlacementCommand command, out string message)
        {
            message = null;
            ...
            switch (command.Kind)
            {
                case SceneSpritePlacementCommandKind.Remove: return RemovePlacement(state, out message);
                case SceneSpritePlacementCommandKind.Cancel: return CancelActivePlacement("Scene sprite placement cancelled.", out message);
                case SceneSpritePlacementCommandKind.Start:
                    if (string.IsNullOrEmpty(command.CandidateToken))
                    {
                        message = "Scene sprite selection is unavailable.";
                        return false;
                    }
                    return StartPlacement(state, command.CandidateToken, out message);
                default:
                    message = "Scene sprite placement command is not supported.";
                    return false;
            }
        }
        ...
        private bool StartPlacement(ScenarioAuthoringState state, string token, out string message)
        {
            ...
            PlacementPickerModel model = GetPickerModel(session, anchor, _sessionStore.CurrentFilePath, state);
            ...
            message = BuildPlacementSummary(candidate, _activePlacement.SnapToGridDefault);
        ...
        private bool CompleteActivePlacement(ScenarioAuthoringState state, ScenarioEditorSession session, out string message)
        {
            ...
            _sceneAssetPreview.RefreshScenePlacements(session.WorkingDefinition, _sessionStore.CurrentFilePath);
            Invalidate();
            ScenarioHierarchyAuthoringContentBuilder.InvalidateLiveObjectCache();
        ...
        private void UpdatePreview(ScenarioDefinition definition, Vector3 worldPoint, bool snapToGrid)
        {
            if (!HasActivePlacement)
                return;

            PlacementResolution resolution = ResolvePlacement(definition, _activePlacement, worldPoint, snapToGrid);
            ...
            bool snapToGrid)
        {
            ...
            {
                worldPoint.z = 0f;
                bool insideBounds = IsFreePlacementWithinBounds(session, worldPoint);
            ...
            if (grid == null
                || !grid.isInitialized
                || !ShelteredScenarioRuntime.TryGetShelterGridCell(worldPoint, out preferredX, out preferredY))
            {
                worldPoint.z = 0f;
                return new PlacementResolution
                {
            ...
            {
                Vector3 position = ShelteredScenarioRuntime.GetShelterGridCellCenter(preferredX, preferredY);
                position.z = 0f;
                return new PlacementResolution
                {
                    Position = position,
            ...
            Vector3 fallback = ShelteredScenarioRuntime.GetShelterGridCellCenter(preferredX, preferredY);
            fallback.z = 0f;
            return new PlacementResolution
            {
                Position = fallback,
            ...
            int gridY)
        {
            ...
            Vector3 position = ShelteredScenarioRuntime.GetShelterGridCellCenter(gridX, gridY);
        ...
        private static bool IsFreePlacementWithinBounds(ActivePlacementSession session, Vector3 worldPoint)
        {
            ShelterRoomGrid grid = ShelterRoomGrid.Instance;
            if (grid == null || session == null || session.Candidate == null || session.Candidate.Sprite == null)
                return true;

            Bounds bounds = CreateSpriteBounds(session.Candidate.Sprite, worldPoint);
            return IsWithinGridBounds(grid, bounds);
        }
        ...
        private static bool OverlapsExistingPlacement(
            ...
            int gridY)
        {
            ScenarioRuntimeIdentityEntry[] markers = ScenarioRuntimeIdentityCatalog.ListSceneSpritePlacements();
            for (int i = 0; markers != null && i < markers.Length; i++)
            {
                ScenarioRuntimeIdentityEntry marker = markers[i];
                if (marker == null || marker.Identity == null
                    || string.Equals(marker.Identity.PlacementId, existingPlacementId, StringComparison.OrdinalIgnoreCase))
                    continue;

                SpriteRenderer renderer = marker.GameObject.GetComponent<SpriteRenderer>() ?? marker.GameObject.GetComponentInChildren<SpriteRenderer>(true);
        ...
        private static bool ResolveSnapToGrid(ScenarioAuthoringState state)
        {
            bool snapToGrid = state == null || state.Settings == null || state.Settings.GetBool("visuals.snap_to_grid", true);
            bool shiftHeld = UnityEngine.Input.GetKey(KeyCode.LeftShift) || UnityEngine.Input.GetKey(KeyCode.RightShift);
            return shiftHeld ? !snapToGrid : snapToGrid;
        }

        private static string BuildPlacementSummary(ScenarioSpriteCatalogService.SpriteCandidate candidate, bool snapToGrid)
        {
            string label = SafeLabel(candidate != null ? candidate.Label : null);
            return snapToGrid
                ? "Placing '" + label + "'. Scene sprites snap to bunker cells; hold Shift to place freely."
                : "Placing '" + label + "'. Scene sprites place freely inside shelter bounds; hold Shift to snap to cells.";
        }
