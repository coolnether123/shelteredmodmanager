...
namespace ShelteredScenarioEditor.Application.Assets{
    ...
    internal sealed partial class ScenarioSpriteSwapAuthoringService
    {
        ...
        {
            public ScenarioRuntimeSpriteTarget Target;
        ...
        internal sealed class CustomEditorModel
        {
            ...
            public ScenarioEditorCharacterTexturePart CharacterPart;
        ...
        private readonly ScenarioEditorSceneAssetPreviewService _sceneAssetPreview;
        ...
        private ScenarioEditorCharacterAppearanceService.PreviewSession _characterPreviewSession;
        ...
        internal ScenarioSpriteSwapAuthoringService(
            ...
            ScenarioEditorSceneAssetPreviewService sceneAssetPreview,
            ...
            ScenarioHoverVisualService hoverVisuals)
        {
            ...
            _sceneAssetPreview = sceneAssetPreview;
        ...
        public CustomEditorModel GetCustomEditorModel(ScenarioAuthoringState state)
        {
            ...
            return new CustomEditorModel
            {
                ...
                CharacterPartLabel = _customEditorSession.IsCharacterEditor
                    ? ScenarioEditorCharacterAppearanceService.BuildPartLabel(_customEditorSession.CharacterPart)
                    : null,
        ...
        private bool OpenPicker(ScenarioAuthoringState state, out string message)
        {
            ...
            if (_characterAppearanceService.TryResolve(state.SelectedTarget, out characterTarget, out message))
            {
                ...
                return OpenCharacterEditor(state, characterTarget, ScenarioEditorCharacterTexturePart.Head, out message);
            }
            characterResolveMessage = message;

            SpritePickerModel model = GetPickerModel(session, state.SelectedTarget, _sessionStore.CurrentFilePath);
        ...
        private bool PreviewCandidate(ScenarioAuthoringState state, string token, out string message)
        {
            ...
            if (!ShelteredScenarioRuntime.TryApplyRuntimeSprite(
        ...
        {
            message = null;
            if (!EnsurePickerOpenForSelection(state, out message))
                return false;
            ...
            {
                baseRuntimeSpriteKey = ShelteredScenarioRuntime.CreateRuntimeSpriteKey(sourceSprite);
            }
            ...
            out string message)
        {
            ...
            {
                message = "The selected " + ScenarioEditorCharacterAppearanceService.BuildPartLabel(part).ToLowerInvariant()
            ...
            _customEditorSession = new CustomEditorSession
            {
                ...
                BaselineTexture = ScenarioEditorCharacterAppearanceService.CopyTexture(editableTexture),
                Texture = editableTexture,
                PreviewSprite = previewSprite,
                ActiveColor = initialColor,
                ActiveBrushIndex = FindMatchingBrushIndex(initialColor),
                ...
                BaseRuntimeSpriteKey = ShelteredScenarioRuntime.CreateRuntimeSpriteKey(editableTexture, sourceId),
                AnimationPlaybackSpeed = 1f,
                LastInteractionX = 0,
                LastInteractionY = 0,
            ...
            message = "Character " + ScenarioEditorCharacterAppearanceService.BuildPartLabel(part).ToLowerInvariant()
                + " editor opened for '" + SafeLabel(target.DisplayName) + "'.";
            return true;
        }

        private bool SwitchCharacterPart(ScenarioAuthoringState state, ScenarioEditorCharacterTexturePart part, out string message)
        {
            message = null;
            ...
            {
                message = "Save or cancel the current " + ScenarioEditorCharacterAppearanceService.BuildPartLabel(_customEditorSession.CharacterPart).ToLowerInvariant()
        ...
        private bool SaveCharacterEditor(ScenarioAuthoringState state, out string message)
        {
            ...
            string packRoot = !string.IsNullOrEmpty(_sessionStore.CurrentFilePath)
                ? Path.GetDirectoryName(_sessionStore.CurrentFilePath)
                : null;
            ...
            try
            {
                ...
                _historyService.RecordVisualChange(
                    definition,
                    "Apply character " + ScenarioEditorCharacterAppearanceService.BuildPartLabel(_customEditorSession.CharacterPart).ToLowerInvariant()
                    + " texture to " + SafeLabel(target.DisplayName));

                string patchMessage;
                string patchId = UpsertPatchSpriteAsset(definition, customTextureId, _customEditorSession.SourceLabel, _sessionStore.CurrentFilePath, out patchMessage);
                ...
                ScenarioEditorCharacterAppearanceService.UpsertAppearance(
                    memberConfig,
                ...
                string applyMessage;
                ShelteredScenarioRuntime.ApplyConfiguredAppearance(
                    definition,
                ...
                ClosePickerState(state, false);
                message = "Saved character " + ScenarioEditorCharacterAppearanceService.BuildPartLabel(_customEditorSession.CharacterPart).ToLowerInvariant()
        ...
        private bool EnsurePickerOpenForSelection(ScenarioAuthoringState state, out string message)
        {
            message = null;
            if (IsPickerBoundToSelection(state))
                return true;
        ...
        private void ReconcilePickerTarget(ScenarioAuthoringState state)
        {
            if (!IsPickerOpen(state)
                || IsPickerBoundToSelection(state))
            {
                return;
            }
        ...
        private static bool IsPickerBoundToSelection(ScenarioAuthoringState state)
        {
            return IsPickerOpen(state)
                && state != null
                && AreSameTarget(state.SelectedTarget, state.SpriteSwapPicker.Target);
        }
        ...
        private bool ImportSpritePngReplacement(ScenarioAuthoringState state, out string message)
        {
            ...
            _sceneAssetPreview.RefreshSpriteSwaps(definition, _sessionStore.CurrentFilePath);
            ClosePickerState(state, false);
            Invalidate();
        ...
        private bool ImportCharacterPngReplacement(ScenarioAuthoringState state, out string message)
        {
            ...
            ScenarioEditorCharacterTexturePart part = _customEditorSession.CharacterPart;
            string partLabel = ScenarioEditorCharacterAppearanceService.BuildPartLabel(part);
            ScenarioEditorCharacterAppearanceService.ResolvedCharacterTarget target;
            if (!TryResolveCharacterEditorTarget(state, out target, out message))
                return false;
            ...
            out string message)
        {
            ...
            try
            {
                ...
                string patchMessage;
                string patchId = UpsertPatchSpriteAsset(definition, customSpriteId, _customEditorSession.SourceLabel, _sessionStore.CurrentFilePath, out patchMessage);
                if (string.IsNullOrEmpty(patchId))
                {
        ...
        private bool ClearActiveSwap(ScenarioAuthoringState state, out string message)
        {
            ...
            SpritePickerModel model = authoringTarget != null
                ? GetPickerModel(session, authoringTarget, _sessionStore.CurrentFilePath)
                : null;
        ...
        private bool CopyActiveSwap(ScenarioAuthoringState state, out string message)
        {
            ...
            _hoverVisuals.SetSecondary(state.SelectedTarget);
        ...
        private bool Undo(ScenarioAuthoringState state, out string message)
        {
            ...
            ReapplyVisualState(definition, _sessionStore.CurrentFilePath);
            Invalidate();
            ...
            out string message)
        {
            ...
            model = GetPickerModel(session, state.SpriteSwapPicker.Target, _sessionStore.CurrentFilePath);
            if (model == null || model.Target == null)
            {
        ...
        private bool EnsurePreviewSession(ScenarioRuntimeSpriteTarget target, out string message)
        {
            message = null;
            if (target == null || !ScenarioEditorSpriteRuntimeResolver.IsAlive(target))
            {
        ...
        private void RestorePreviewSession()
        {
            ...
            ShelteredScenarioRuntime.TryApplyRuntimeSprite(
                _previewSession.TargetPath,
        ...
        private ScenarioEditCategory[] ResolveUndoScope(ScenarioAuthoringState state)
        {
            ...
            if (state.ActiveStage == ShelteredScenarioEditor.Domain.Stages.ScenarioStageKind.BunkerInside)
        ...
        private void AttachAnimationFramesIfAvailable(
            ScenarioRuntimeSpriteTarget target,
            ...
            SpritePickerModel model)
        {
            ...
            _customEditorSession.AnimationFrameIndex = FindAnimationFrameIndex(frames, ShelteredScenarioRuntime.CreateRuntimeSpriteKey(target.CurrentSprite));
        ...
        private bool TryResolveCharacterEditorTarget(
            ScenarioAuthoringState state,
            out ScenarioEditorCharacterAppearanceService.ResolvedCharacterTarget target,
            out string message)
        {
            target = null;
            message = null;
        ...
        {
            _sceneAssetPreview.RefreshAll(definition, scenarioFilePath);
            ...
            for (int i = 0; definition != null && definition.FamilySetup != null && family != null && i < definition.FamilySetup.Members.Count && i < family.Count; i++)
            {
                ...
                if (config != null && member != null)
                    ShelteredScenarioRuntime.ApplyConfiguredAppearance(definition, scenarioFilePath, config, member, out ignored);
            }
        }
        ...
        private void ApplyCustomEditorPreview(ScenarioAuthoringState state)
        {
            ...
            if (IsAnimationEditor())
                _previewHost.TryPreviewRuntimeSpriteFrame(
                    model.Target.TargetPath,
                    model.Target.Kind,
                    _customEditorSession.PreviewSprite);
            else
                ShelteredScenarioRuntime.TryApplyRuntimeSprite(
                    model.Target.TargetPath,
                    model.Target.Kind,
        ...
        private void UpdateWorldAnimationPreview(ScenarioAuthoringState state)
        {
            ...
            _previewHost.TryPlayRuntimeSpriteAnimation(
                model.Target.TargetPath,
                model.Target.Kind,
                sprites.ToArray(),
                durations.ToArray(),
        ...
        private void StopWorldAnimationPreview(ScenarioAuthoringState state)
        {
            ...
            _previewHost.StopRuntimeSpriteAnimation(targetPath, targetKind);
        }

        private void BeginCustomEditorCameraSession(ScenarioAuthoringState state, Rect editorWindowRect)
        {
            if (_editorCamera != null && state != null && state.SpriteSwapPicker != null)
                _editorCamera.BeginPixelEditorSession(
                    state.SpriteSwapPicker.Target,
                    state.SpriteSwapPicker.TargetPath,
                    editorWindowRect);
        }

        private void EndCustomEditorCameraSession()
        {
            if (_editorCamera != null)
                _editorCamera.EndPixelEditorSession();
        }
        ...
        {
            return ScenarioAutomationIdCodec.DecodeToken(encoded);
        }
