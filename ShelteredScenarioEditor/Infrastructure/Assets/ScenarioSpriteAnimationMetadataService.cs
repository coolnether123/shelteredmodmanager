...
namespace ShelteredScenarioEditor.Infrastructure.Assets{
    internal sealed class ScenarioSpriteAnimationMetadataService
    {
        ...
        public AnimationMetadata Resolve(ScenarioRuntimeSpriteTarget target)
        {
            if (target == null || target.Kind != ScenarioSpriteTargetComponentKind.SpriteRenderer || target.SpriteRenderer == null)
                return null;
            ...
            string currentKey = ShelteredScenarioRuntime.CreateRuntimeSpriteKey(target.CurrentSprite);
        ...
        private static AnimationMetadata SampleClip(SpriteRenderer sourceRenderer, AnimationClip clip, string currentKey)
        {
            ...
            try
            {
                ...
                for (float time = 0f; time <= clip.length + (step * 0.5f); time += step)
                {
                    ...
                    string key = ShelteredScenarioRuntime.CreateRuntimeSpriteKey(sampled);
