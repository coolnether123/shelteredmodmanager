...
namespace ShelteredScenarioEditor.Infrastructure.Unity{
    ...
    internal static class ScenarioInventoryItemCatalog
    {
        ...
        public static ScenarioInventoryItemCatalogEntry Resolve(string itemId)
        {
            ...
            if (!ShelteredContent.Runtime.ResolveItemType(itemId, out type))
        ...
        {
            for (int i = 0; i < ShelteredContent.RegisteredItems.Count; i++)
            {
                ItemDefinitionSnapshot item = ShelteredContent.RegisteredItems[i];
                if (item != null && item.CustomTypeId.HasValue && item.CustomTypeId.Value == (int)type)
                    return item.Id;
            }
        ...
        private static List<ItemManager.ItemType> BuildCatalogTypes()
        {
            ...
            {
                ItemDefinitionSnapshot item = ShelteredContent.RegisteredItems[i];
                if (item != null && item.CustomTypeId.HasValue)
                    AddType(types, seen, (ItemManager.ItemType)item.CustomTypeId.Value);
            }
        ...
        private static int ComputeRuntimeSignature()
        {
            unchecked
            {
                ...
                int registeredCount = ShelteredContent.RegisteredItems.Count;
                for (int i = 0; i < ShelteredContent.RegisteredItems.Count; i++)
                {
                    ItemDefinitionSnapshot item = ShelteredContent.RegisteredItems[i];
                    if (item != null && item.CustomTypeId.HasValue)
                        hash ^= (item.CustomTypeId.Value * 7919);
                }
        ...
        private static int IndexOf(List<ScenarioInventoryItemCatalogEntry> catalog, string itemId)
        {
            ...
            bool hasType = ShelteredContent.Runtime.ResolveItemType(itemId, out currentType);
