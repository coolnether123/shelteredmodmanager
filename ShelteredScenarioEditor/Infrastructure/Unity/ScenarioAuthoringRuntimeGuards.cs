...
namespace ShelteredScenarioEditor.Infrastructure.Unity{
    internal static class ScenarioAuthoringRuntimeGuards
    {
        ...
        {
            return ScenarioCompositionRoot.Resolve<ScenarioAuthoringSessionLifecycleService>().IsEditingDraftActive();
        }

        public static bool IsAuthoringPending()
        {
            return ScenarioCompositionRoot.Resolve<ScenarioAuthoringSessionLifecycleService>().HasPendingDraftLaunch();
        }
        ...
        public static bool ShouldMaintainPausedSimulation()
        {
            ...
            // The world-loading editor shell is intentionally interactive while
            // vanilla completes its clock, camera, and transition-fade warmup.
            if (IsAuthoringPending())
                return false;
