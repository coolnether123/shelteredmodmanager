...
namespace ShelteredScenarioEditor.Diagnostics
{
    internal static class ScenarioMapUxVerification
    {
        ...
        private static void VerifyLootPreview(ScenarioValidationResult result)
        {
            ...
            Assert(string.Equals(LootRollSignature(preview), LootRollSignature(repeated), StringComparison.Ordinal),
                "MAPUX exact loot preview must be deterministic for a supplied simulation seed.", result);
        ...
        private static string LootRollSignature(ScenarioMapLootPreview preview)
        {
            StringBuilder signature = new StringBuilder();
            for (int i = 0; preview != null && preview.ExactRoll != null && i < preview.ExactRoll.Count; i++)
            {
                if (i > 0) signature.Append('|');
                signature.Append(preview.ExactRoll[i].ItemId).Append(':').Append(preview.ExactRoll[i].Quantity);
            }
        ...
        private static void VerifyTerrainBrushDraft(ScenarioValidationResult result)
        {
            ...
            MapTerrainPatchDefinition round = service.PaintTerrainArea(
                session, 10, 6, ShelteredScenarioAuthoring.GeneratedBlendTerrainId, MapTerrainBrushShape.Circle, 5);
