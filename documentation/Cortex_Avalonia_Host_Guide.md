...
When a Unity host launches Avalonia externally, the same pipe-name convention is reused. If `Cortex.Host.Avalonia.exe` is unavailable but `Cortex.Host.Avalonia.dll` exists, the launcher falls back to `dotnet <dll>` and reports a specific startup failure when `dotnet.exe` is not available on `PATH`.
