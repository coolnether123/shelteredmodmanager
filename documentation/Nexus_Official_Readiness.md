...
- SMM identifies every API request with `Application-Name`, `Application-Version`, and `User-Agent`. Authenticated requests use `Authorization: Bearer` exclusively.
...
- Nexus OAuth sessions are validated through the v1 user-validation endpoint before the UI reports a connected account.
- Historical plaintext or DPAPI-protected personal-key settings are removed during settings load and save without being decrypted or used. Other settings are preserved.
- OAuth refresh failure or an API `401 Unauthorized` clears the OAuth session and requires reconnection; no other credential is attempted.
