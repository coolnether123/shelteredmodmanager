    ...
    public static class ManagedShutdown_Interceptor
    {
        public static bool Prefix(MainMenuPanel __instance, int response)
        {
            ...
            if (response == 1)
            {
                ...
                SaveExitTracker.Mark("OnMessageBoxClosed(response=1)", "User confirmed Save & Exit");

                // Reset save-completion flag for new quit sequence
                PlatformSaveProxy.ResetStatus();
                SaveExitTracker.Mark("IsQuitting set true");
            ...
            public static bool Prefix(SaveManager __instance)
            {
                ...
                if (PluginRunner.IsQuitting && !__instance.isSaving && !__instance.isLoading)
                {
                    if (Time.realtimeSinceStartup >= _nextPassthroughLogAt)
                    {
                        ...
                        SaveExitTracker.Mark("SaveManager.Update passthrough", "isSaving=false, isLoading=false");
                    }
                    return true;
                }
                return true;
