...
namespace Cortex.Plugin.Harmony.Runtime
{
    internal sealed class HarmonyRuntimeInspectionService
    {
        private static readonly CortexLogger Log = CortexLog.ForSource("Cortex.Harmony");
        private readonly Func<Assembly> _harmonyAssemblyResolver;

        public HarmonyRuntimeInspectionService()
            : this(ResolveHarmonyAssembly)
        {
        }

        internal HarmonyRuntimeInspectionService(Func<Assembly> harmonyAssemblyResolver)
        {
            _harmonyAssemblyResolver = harmonyAssemblyResolver ?? ResolveHarmonyAssembly;
        }

        public bool IsAvailable
        {
            get { return GetHarmonyAssembly() != null; }
        }
        ...
        public string UnavailableReason
        {
            get
            {
                return IsAvailable
                    ? string.Empty
                    : "Harmony module is disabled because 0Harmony is not available in the active runtime.";
            }
        }
        ...
        public HarmonyPatchSnapshot CaptureSnapshot()
        {
            ...
            if (!IsAvailable)
            {
                ...
                snapshot.StatusMessage = UnavailableReason;
                return snapshot;
            }
        ...
        public HarmonyMethodPatchSummary Inspect(HarmonyPatchInspectionRequest request)
        {
            if (!IsAvailable)
            {
                return BuildUnresolvedSummary(request, UnavailableReason);
            }
            ...
            if (method == null)
            {
                ...
                return BuildUnresolvedSummary(request, "Method could not be resolved from the current runtime.");
            }
        ...
        private static HarmonyMethodPatchSummary BuildSummary(MethodBase method)
        {
            ...
            var patches = global::HarmonyLib.Harmony.GetPatchInfo(method);
        ...
        private static HarmonyMethodPatchSummary BuildUnresolvedSummary(HarmonyPatchInspectionRequest request, string conflictHint)
        {
            var summary = new HarmonyMethodPatchSummary();
            summary.CapturedUtc = DateTime.UtcNow;
            ...
            summary.ConflictHint = conflictHint ?? string.Empty;
            summary.Order = new HarmonyPatchOrderExplanation[0];
            summary.IsPatched = false;
            return summary;
        }

        private Assembly GetHarmonyAssembly()
        {
            try
            {
                return _harmonyAssemblyResolver();
            }
            catch
            {
                return null;
        ...
        private static Assembly ResolveHarmonyAssembly()
        {
            ...
            for (var i = 0; i < assemblies.Length; i++)
            {
                ...
                var simpleName = name != null ? name.Name ?? string.Empty : string.Empty;
        ...
        private static string GetAssemblyPath(MethodBase method)
        {
            try
            {
                ...
                var codeBase = assembly.CodeBase ?? string.Empty;
                if (codeBase.StartsWith("file:///", StringComparison.OrdinalIgnoreCase))
                {
                    Uri codeBaseUri;
                    if (Uri.TryCreate(codeBase, UriKind.Absolute, out codeBaseUri) && codeBaseUri.IsFile)
                    {
                        return codeBaseUri.LocalPath ?? string.Empty;
                    }
                }
