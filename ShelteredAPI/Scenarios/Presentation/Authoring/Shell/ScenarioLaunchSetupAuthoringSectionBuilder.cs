...
namespace ShelteredAPI.Scenarios.Presentation.Authoring.Shell
{
    internal static class ScenarioLaunchSetupAuthoringSectionBuilder
    {
        ...
        public static ScenarioAuthoringInspectorSection Build(ScenarioDefinition definition)
        {
            ...
            if (setup.Mode != ScenarioLaunchSetupMode.FullSetup)
            {
                for (int i = 0; i < Ids.Length; i++)
                {
                    ...
                    items.Add(Item.ActionItem(Item.Action("launch_setup.value." + Ids[i] + ".-1", "-", "Use the previous authored value.", true, false, "-")));
                    items.Add(Item.ActionItem(Item.Action("launch_setup.value." + Ids[i] + ".1", "+", "Use the next authored value.", true, false, "+")));
                    if (setup.Mode == ScenarioLaunchSetupMode.Guided)
                    {
                        bool selectable = category == null || category.PlayerSelectable;
                        items.Add(Item.ActionItem(Item.Action("launch_setup.selectable." + Ids[i], selectable ? "Player choice" : "Authored lock",
        ...
        {
            items.Add(Item.ActionItem(Item.Action("launch_setup.mode." + mode, label, hint, true, current == mode, "PLAY")));
        }
