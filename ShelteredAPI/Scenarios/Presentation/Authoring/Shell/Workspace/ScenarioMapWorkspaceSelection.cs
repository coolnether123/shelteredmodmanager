...
namespace ShelteredAPI.Scenarios.Presentation.Authoring.Shell
{
    ...
    internal static class ScenarioMapWorkspaceSelection
    {
        ...
        public static void SelectLocation(ScenarioAuthoringState state, ScenarioDefinition definition, MapLocationDefinition location)
        {
            ...
            rendererState.SetWorkspaceSubtab(WorkspaceId, MainSubtabId);
            rendererState.SetWorkspaceSelection(WorkspaceId, MainSubtabId, entityId);
            rendererState.SetWorkspaceNarrowPane(WorkspaceId, MainSubtabId, true);
        }

        public static void ClearLocationSelection(ScenarioAuthoringState state)
        {
            if (state != null)
                state.MapSelectedLocationId = null;
            ScenarioAuthoringRendererInteractionState.Instance.SetWorkspaceSelection(WorkspaceId, MainSubtabId, null);
        }
        ...
        private static string BuildEntityId(string prefix, string id, int matchingIds, int index)
        {
            return !string.IsNullOrEmpty(id) && matchingIds == 1
                ? prefix + ":id:" + ScenarioAuthoringActionCodec.EncodeToken(id)
        ...
        private static bool TryParseEntityId(string entityId, string prefix, out string id, out int index)
        {
            ...
            {
                id = ScenarioAuthoringActionCodec.DecodeToken(entityId.Substring(idPrefix.Length));
