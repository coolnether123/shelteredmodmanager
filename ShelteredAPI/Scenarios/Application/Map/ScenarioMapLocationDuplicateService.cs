...
namespace ShelteredAPI.Scenarios.Application.Map
{
    internal static class ScenarioMapLocationDuplicateService
    {
        ...
        {
            return string.IsNullOrEmpty(sourceId) ? null : ModePrefix + ScenarioAuthoringActionCodec.EncodeToken(sourceId);
        }
        ...
        public static bool TryReadSourceId(string mode, out string sourceId)
        {
            ...
            if (string.IsNullOrEmpty(mode) || !mode.StartsWith(ModePrefix, StringComparison.OrdinalIgnoreCase))
                return false;
            sourceId = ScenarioAuthoringActionCodec.DecodeToken(mode.Substring(ModePrefix.Length));
