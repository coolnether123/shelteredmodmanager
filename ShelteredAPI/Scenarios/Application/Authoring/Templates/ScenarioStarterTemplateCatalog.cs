...
namespace ShelteredAPI.Scenarios.Application.Authoring.Templates
{
    internal sealed class ScenarioStarterTemplate
    {
        ...
        internal ScenarioStarterTemplate(string key, string title, string teaches, string bundledXml, ScenarioDefinitionSerializer serializer)
        {
            Key = key;
            Title = title;
            Teaches = teaches;
    ...
    {
        private static readonly ScenarioDefinitionSerializer Serializer = new ScenarioDefinitionSerializer();
    ...
    internal static class ScenarioStarterTemplateSummary
    {
        ...
        private static int FindSurvivalVictoryDay(ScenarioDefinition definition)
        {
            for (int i = 0; definition.WinLossConditions != null && i < definition.WinLossConditions.WinConditions.Count; i++)
            {
                ...
                if (condition == null || !string.Equals(condition.Type, "surviveDays", StringComparison.OrdinalIgnoreCase))
                    continue;
                for (int p = 0; p < condition.Properties.Count; p++)
                {
                    int day;
                    ScenarioProperty property = condition.Properties[p];
                    if (property != null && (string.Equals(property.Key, "day", StringComparison.OrdinalIgnoreCase) || string.Equals(property.Key, "days", StringComparison.OrdinalIgnoreCase))
                        && int.TryParse(property.Value, NumberStyles.Integer, CultureInfo.InvariantCulture, out day))
                        return day;
                }
            }
            return 0;
