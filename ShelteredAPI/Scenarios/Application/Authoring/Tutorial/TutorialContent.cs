...
namespace ShelteredAPI.Scenarios.Application.Authoring.Tutorial{
    internal static class TutorialContent
    {
        ...
        private static readonly ScenarioAuthoringHelpPage[] HelpPages =
        {
            ...
            new ScenarioAuthoringHelpPage(TopicMap, "Map", "Open the real expedition map to select vanilla towns and regions and edit them directly: name, kind, icon, danger, whether they can be searched, and what starts visible or discovered. Your edits are captured into the scenario, so the shared vanilla map is never changed. Use Place, Move, or Duplicate on authored locations; filters and the legend help separate authored and vanilla markers. Preview deterministic loot and encounter data before playtest. Making brand-new regions in empty map cells is intentionally blocked to keep the vanilla map stable, and a few encounter fields are stored for reference but are not applied at runtime yet.", ScenarioStageKind.Map, ScenarioAuthoringWindowIds.Map, null),
