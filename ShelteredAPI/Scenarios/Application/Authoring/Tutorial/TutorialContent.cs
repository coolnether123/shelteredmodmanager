...
namespace ShelteredAPI.Scenarios.Application.Authoring.Tutorial{
    internal static class TutorialContent
    {
        ...
        private static readonly TutorialStep[] Steps =
        {
            ...
            new TutorialStep(3, "tool_workspace", "TOOL WORKSPACE BASICS", "Use the tool rail for what you are editing. Selection inspects live targets; Objects opens placement and build tools.", "SELECT TOOL", "tool:" + ScenarioAuthoringTool.Select, null, ScenarioStageKind.BunkerInside, null),
        ...
        private static readonly ScenarioAuthoringHelpPage[] HelpPages =
        {
            ...
            new ScenarioAuthoringHelpPage(TopicArtPixelEditor, "Art & Pixel Editor", "Art tools can replace existing target sprites, place scene sprites, import PNG assets, and edit custom sprite drafts before applying them to the scenario. Animated sprite targets keep the vanilla clip frame count and timing fixed; the editor can replace existing frames but cannot add, remove, or retime frames.", ScenarioStageKind.BunkerInside, ScenarioAuthoringWindowIds.PixelEditor, TourEditSprite),
