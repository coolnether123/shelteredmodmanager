...
namespace ShelteredAPI.Scenarios.Application.Authoring.Tutorial{
    internal static class TutorialContent
    {
        ...
        private static readonly ScenarioAuthoringHelpPage[] HelpPages =
        {
            ...
            new ScenarioAuthoringHelpPage(TopicSelectionCycling, "Selection & Cycling", "Left-click focuses live scene targets for editing; right-click hands the click to the vanilla world so you can open real objects and menus. When multiple targets overlap, the selection stack can cycle or select a specific row; clearing selection drops selected and multi-selected targets.", ScenarioStageKind.BunkerInside, ScenarioAuthoringWindowIds.Hierarchy, TourEditorBasics),
            ...
            new ScenarioAuthoringHelpPage(TopicSupplies, "Supplies", "Start supplies decide the first-day pressure: food, water, fuel, and medicine. Open the shelter storage window to stock supplies in the real panel; what you leave in storage syncs straight into the draft. If an issue blocks testing, add missing required categories in Supplies or check scheduled starts/flows in Scheduled Stock.", ScenarioStageKind.InventoryStorage, ScenarioAuthoringWindowIds.Stockpile, null),
            new ScenarioAuthoringHelpPage(TopicMap, "Map", "Open the real expedition map to select vanilla towns and regions and edit them directly: name, kind, icon, danger, whether they can be searched, and what starts visible or discovered. Your edits are captured into the scenario, so the shared vanilla map is never changed. Use the Place and Move map modes to drop and relocate your own authored locations, give a location deterministic loot (item counts, drop chance, weighting, hidden finds, and unlock items), and set its encounter chances. Making brand-new regions in empty map cells is intentionally blocked to keep the vanilla map stable, and a few encounter fields are stored for reference but are not applied at runtime yet.", ScenarioStageKind.Map, ScenarioAuthoringWindowIds.Map, null),
            ...
            new ScenarioAuthoringHelpPage(TopicPublish, "Package / Export", "Package / Export is your health checkpoint. It collects blocking errors, warnings, unsupported-feature checks, and package state before creating a local export. Resolve blockers by opening the linked source pages shown on each issue.", ScenarioStageKind.Publish, ScenarioAuthoringWindowIds.Publish, null),
