...
namespace ShelteredAPI.Scenarios.Infrastructure.Runtime
{
    internal sealed class ScenarioMapLootPreview
    {
        ...
        {
            ExactRoll = new List<MapLootProjectionEntry>();
        ...
        public List<MapLootProjectionEntry> ExactRoll { get; private set; }
    ...
    internal static class ScenarioMapLootPreviewService
    {
            ...
            int rollCount)
        {
            ...
            preview.ExactRoll.AddRange(ScenarioMapProjectionApplyService.PlanLootRolls(definition, location, table));
            ...
            MapLootTableDefinition table)
        {
            ...
            for (int rollIndex = 0; rollIndex < preview.RollCount; rollIndex++)
            {
                ...
                List<MapLootProjectionEntry> roll = ScenarioMapProjectionApplyService.PlanLootRolls(definition, location, table, rollSeed);
                for (int itemIndex = 0; roll != null && itemIndex < roll.Count; itemIndex++)
                {
                    MapLootProjectionEntry item = roll[itemIndex];
