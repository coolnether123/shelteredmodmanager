...
namespace Cortex.Services.Semantics.Hover
{
    ...
    internal sealed class EditorHoverService : IEditorHoverService
    {
        ...
        private readonly EditorHoverTargetFactory _targetFactory;
        private readonly EditorHoverContentMapper _contentMapper = new EditorHoverContentMapper();
        ...
        public EditorHoverService(IEditorContextService contextService)
        {
            ...
            _targetFactory = new EditorHoverTargetFactory(contextService);
        }
        ...
        {
            return _targetFactory.TryCreateInteractionTarget(session, state, editingEnabled, absolutePosition, out target);
        }
        ...
        {
            var published = _targetFactory.TryCreateSourceHoverTarget(session, state, editingEnabled, surfaceId, paneId, surfaceKind, absolutePosition, anchorRect, tokenClassification, out hoverTarget);
            LogHoverTargetCreated(
                ...
                absolutePosition,
                hoverTarget != null ? hoverTarget.HoverKey ?? string.Empty : BuildHoverKey(session, hoverTarget != null ? hoverTarget.Target : null),
            ...
            out EditorHoverTarget hoverTarget)
        {
            ...
            var published = _targetFactory.TryCreateReadOnlyHoverTarget(session, state, surfaceId, paneId, surfaceKind, absolutePosition, line, column, tokenText, canNavigateToDefinition, tokenClassification, anchorRect, out hoverTarget);
            LogHoverTargetCreated(
                published,
                surfaceId,
                surfaceKind,
                ...
                hoverTarget != null && hoverTarget.Target != null ? hoverTarget.Target.SymbolText ?? string.Empty : tokenText,
        ...
        {
            return _targetFactory.ResolveHoverResponse(state, hoverKey);
        }
        ...
        public LanguageServiceHoverResponse ResolveHoverResponse(CortexShellState state, DocumentSession session, EditorCommandTarget target)
        {
            ...
            return _targetFactory.ResolveHoverResponse(state, session, target);
        }
        ...
        {
            return _contentMapper.BuildHoverContent(hoverTarget, response);
        }
        ...
        {
            return new EditorHoverContentMapper().BuildRenderModel(hoverTarget, hoverContent);
        }
        ...
        {
            return new EditorHoverContentMapper().CountParts(parts);
        }

        private static int CountInteractiveParts(EditorResolvedHoverContent hoverContent)
        {
            return new EditorHoverContentMapper().CountInteractiveParts(hoverContent);
        }

        private static int CountSections(EditorResolvedHoverContent hoverContent)
        {
            return new EditorHoverContentMapper().CountSections(hoverContent);
        }
