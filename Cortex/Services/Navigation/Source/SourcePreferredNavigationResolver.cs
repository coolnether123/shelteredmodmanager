...
{
    internal interface ISourcePreferredNavigationResolver
    {
        bool TryResolveFromSymbol(CortexShellState state, ISourceLookupIndex sourceLookupIndex, LanguageSymbolNavigationRequest request, out string sourceDocumentPath);
        bool TryResolveFromTypeName(CortexShellState state, ISourceLookupIndex sourceLookupIndex, string fullTypeName, out string sourceDocumentPath);
    }

    internal sealed class SourcePreferredNavigationResolver : ISourcePreferredNavigationResolver
    {
        public bool TryResolveFromSymbol(CortexShellState state, ISourceLookupIndex sourceLookupIndex, LanguageSymbolNavigationRequest request, out string sourceDocumentPath)
        {
            ...
            {
                sourceDocumentPath = request.DefinitionDocumentPath;
                return true;
            }

            var fullTypeName = ExtractFullTypeName(
        ...
        public bool TryResolveFromTypeName(CortexShellState state, ISourceLookupIndex sourceLookupIndex, string fullTypeName, out string sourceDocumentPath)
        {
            sourceDocumentPath = string.Empty;
            ...
            var typeLeafName = SourceNavigationLineResolver.GetTypeLeafName(fullTypeName);
            if (string.IsNullOrEmpty(typeLeafName))
            {
                return false;
            }

            var roots = SourceRootSetBuilder.Build(
                state != null ? state.SelectedProject : null,
                state != null ? state.Settings : null,
                SourceRootFlags.ProjectSource | SourceRootFlags.ProjectDirectory | SourceRootFlags.Workspace | SourceRootFlags.Additional);
            if (roots.Count == 0)
            {
                return false;
            }
        ...
        private static int ScoreCandidate(CortexShellState state, string candidatePath, string fullTypeName, string typeLeafName)
        {
            ...
            var text = SourceNavigationLineResolver.ReadAllTextSafe(candidatePath);
