...
Assert-Contains "unified custom scenario library" $scenarioBookDataSource "entry == null \|\| entry\.Source != ScenarioCatalogSource\.Modded.*BuildLibraryScenarioDetail\(entry\)" "the entry library must include every modded playable scenario without base-mode grouping or vanilla archive rows."
