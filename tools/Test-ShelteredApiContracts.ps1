...
Assert-Contains "scenario world event runtime" $scheduledWorldEventRuntime "NpcVisitManager.*m_pendingSpawns.*StartBreach.*Obj_Radio" "WorldEvent runtime must bridge NPC visits, raids, and broadcast control through the researched vanilla seams."
...
Assert-Contains "scenario world event suppression" $scenarioWorldEventRuntimeState "SuppressRandomVisitors.*SuppressBinman.*SuppressRaids.*SuppressStasisVisitors.*SuppressRadioBroadcastOdds" "runtime state must expose each vanilla world-event suppression category."
