...
Assert-Contains "scenario end-game presenter injection" $scenarioWinLossOutcomeService 'IScenarioEndGamePresenter.*_endGamePresenter\.TryPresent\(success' "resolved outcomes must use the injected presentation seam."
Assert-Contains "scenario end-game presentation retry" $scenarioWinLossOutcomeService '_presentationPending.*if \(_presentationPending\).*PresentOutcome\(_pendingPresentationSuccess\).*_presentationPending = false' "a resolved outcome must retry presentation until the selected end-game target is ready."
