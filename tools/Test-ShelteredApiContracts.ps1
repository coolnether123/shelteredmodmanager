...
Assert-Contains "custom scenario hub avoids vanilla summary overlap" $scenarioSelectionBrowserController "Bounds bounds = NGUIMath\.CalculateRelativeWidgetBounds\(source\.transform, true\).*float width = bounds\.size\.x > 1f \? bounds\.size\.x : 320f;.*float offsetX = Mathf\.Clamp\(width \+ 360f, 620f, 720f\).*sourcePosition\.x \+ offsetX" "the Custom Scenarios hub must be positioned from the actual vanilla button width with a bounded gap clear of stock summary content."
