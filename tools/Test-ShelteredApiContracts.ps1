...
Assert-Contains "draft details duplicate action" $scenarioBookRenderer '"DuplicateDraft",\s*"Duplicate Draft"' "draft details must expose the harness-visible Duplicate Draft action."
Assert-Contains "confirm-free details duplication" $scenarioBookPanel 'HandleDraftDuplicateRequested.*?_actions\.DuplicateDraft\(_selectedScenario.*?_selectedScenario\s*=\s*new ScenarioCatalogEntry.*?StartDataRefresh' "non-destructive details duplication must use the repository-backed action, select its result, and refresh the catalog."
...
Assert-Contains "fresh installed runtime journal scope" $scenarioRuntimeStateService 'binding\.RunId.*"run:" \+ binding\.RunId.*"legacy-normal"' "new installed runs must not reuse the prior run's execution journal or outcome state."
