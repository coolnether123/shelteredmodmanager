...
Assert-Contains "flash-free book close ordering" $scenarioBookPanel "if \(restoreUnderlyingPanel\)\s*RestoreUnderlyingPanel\(\);.*overlay\.SetActive\(false\);" "closing the book must restore vanilla chrome before hiding the overlay."
