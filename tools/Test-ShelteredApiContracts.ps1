...
Assert-Contains "authoring inventory write-through" $scenarioAuthoringCaptureService "AdoptCurrentDraftAsProjected\(session,\s*""capture live inventory""" "Explicit live inventory capture must reset the projection baseline without auto-capturing later edits."
...
Assert-Contains "authoring inventory contracts" $scenarioVerification "VerifyInventoryProjectionReconciliation.*PlanProjectionDeltas.*BuildProjectionSeed.*ShouldApplyRandomStartOverride" "Scenario verification must cover inventory projection delta, idempotency, live-extra policy, and OverrideRandomStart behavior."
Assert-Contains "authoring inventory UI" $scenarioAuthoringPresentationBuilder "Live Shelter Inventory - Native Storage.*Starting Items - Written to Storage" "Supplies UI must no longer label live shelter inventory as reference-only."
