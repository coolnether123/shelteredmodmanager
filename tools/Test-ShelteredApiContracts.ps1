...
$scenarioPlaytestVanillaUiService = Read-RepoFile "ShelteredAPI\Scenarios\Infrastructure\Unity\ScenarioPlaytestVanillaUiService.cs"
...
$scenarioLiveShelterObjectCatalog = Read-RepoFile "ShelteredAPI\Scenarios\Application\Authoring\ScenarioLiveShelterObjectCatalog.cs"
$scenarioAuthoringSelectionService = Read-RepoFile "ShelteredAPI\Scenarios\Application\Authoring\ScenarioAuthoringSelectionService.cs"
$scenarioWorldObjectDisplayNames = Read-RepoFile "ShelteredAPI\Scenarios\Application\Authoring\ScenarioWorldObjectDisplayNameResolver.cs"
$scenarioHierarchyContentBuilder = Read-RepoFile "ShelteredAPI\Scenarios\Presentation\Authoring\Shell\ScenarioHierarchyAuthoringContentBuilder.cs"
...
$scenarioAuthoringPauseService = Read-RepoFile "ShelteredAPI\Scenarios\Infrastructure\Unity\ScenarioAuthoringPauseService.cs"
$scenarioOpeningCutsceneAuthoringService = Read-RepoFile "ShelteredAPI\Scenarios\Application\Authoring\ScenarioOpeningCutsceneAuthoringService.cs"
...
$scenarioAuthoringTooltipRenderer = Read-RepoFile "ShelteredAPI\Scenarios\Presentation\Authoring\Shell\Rendering\ScenarioAuthoringShellTooltipImguiRenderer.cs"
$scenarioAuthoringAnimationService = Read-RepoFile "ShelteredAPI\Scenarios\Presentation\UiKit\Animation\ScenarioAuthoringShellAnimationService.cs"
$scenarioStageNavigation = Read-RepoFile "ShelteredAPI\Scenarios\Presentation\Authoring\Shell\StageNavigationViewModelBuilder.cs"
$scenarioAuthoringWindowRegistry = Read-RepoFile "ShelteredAPI\Scenarios\Presentation\Authoring\Windows\ScenarioAuthoringWindowRegistry.cs"
$scenarioRendererActionManifest = Read-RepoFile "ShelteredAPI\Scenarios\Presentation\Authoring\Shell\ScenarioAuthoringRendererActionManifest.cs"
...
$scenarioUiAtlasSkin = Read-RepoFile "ShelteredAPI\Scenarios\Presentation\UiKit\Textures\ScenarioUiAtlasSkin.cs"
...
Assert-Contains 'test console hides consumed actions from upcoming' $scenarioTestConsoleContent 'BuildUpcomingItems\(ScenarioDefinition definition, ScenarioRuntimeState runtimeState, bool active\).*!IsOnceConsumed\(runtimeState, action\)' 'Upcoming actions should omit succeeded once-only entries.'
...
$scenarioPublishContent = Read-RepoFile "ShelteredAPI\Scenarios\Presentation\Authoring\Shell\ScenarioPublishAuthoringContentBuilder.cs"
Assert-Contains 'runtime pending excludes vanilla story routes' $scenarioPublishContent '!includeStoryRoutes && entry != null && string\.Equals\(entry\.Source, "flow".*BuildTimelineItems\(definition, ScenarioPublishAuthoringContentBuilder\.GetRuntimeState\(\), _timelineBuilder, false\)' 'The pending-action panel should not mislabel vanilla dialogue routes as scheduler-owned pending actions.'
...
Assert-Contains "legacy family-member conversation binding" $scenarioActorResolver 'legacyFamilyBinding = string\.Equals\(actorRef\.BindingType, "FamilyMember".*actorRef\.BindingKey, candidate\.firstName.*actorRef\.BindingKey, fullName' "Legacy starter-template FamilyMember bindings must resolve by authored family name so timeline conversations can run."
...
Assert-Contains "authoring base-mode reload session refresh" $scenarioAuthoringBootstrapService "sameLaunchPending.*_pendingSession\.BaseMode == baseMode.*_pendingSession\.LaunchSaveType == launchSaveType" "A same-draft pending session may be reused only when its base mode and launch save type still match the saved draft."
Assert-Contains "authoring base-mode reload session refresh" $scenarioAuthoringBootstrapService "if \(sameLaunchPending\).*Reusing pending existing draft bootstrap" "Same-draft pending bootstrap reuse must be gated by the complete launch identity instead of draft id alone."
Assert-Contains "authoring base-mode reload session refresh" $scenarioAuthoringBootstrapService "Replacing pending draft bootstrap.*base=.*saveType=" "Base-mode reload diagnostics must record the stale and replacement launch identities."
...
$scenarioAuthoringBootstrapPatches = Read-RepoFile "ShelteredAPI\Scenarios\Infrastructure\Harmony\ScenarioAuthoringBootstrapPatches.cs"
$scenarioMapAuthoringPatches = Read-RepoFile "ShelteredAPI\Scenarios\Infrastructure\Harmony\ScenarioMapAuthoringPatches.cs"
$scenarioStorageAuthoringPatches = Read-RepoFile "ShelteredAPI\Scenarios\Infrastructure\Harmony\ScenarioStorageAuthoringPatches.cs"
$scenarioVanillaInteractionPatches = Read-RepoFile "ShelteredAPI\Scenarios\Infrastructure\Harmony\ScenarioVanillaInteractionPatches.cs"
...
Assert-NotContains "custom library excludes vanilla rows" $scenarioBookDataSource "entry\.Source != ScenarioCatalogSource\.Vanilla.*AddLibraryScenarioRow\(rows, entry\)" "the custom library must not duplicate the stock Surrounded and Stasis entries."
Assert-Contains "custom grouped paths exclude vanilla rows" $scenarioBookDataSource "IsPlayableScenarioMode\(ScenarioCatalogEntry entry, ScenarioBaseGameMode mode\).*return entry\.Source == ScenarioCatalogSource\.Modded" "legacy grouped library paths must also remain authored-scenario only."
...
Assert-Contains "scenario book deferred runtime graph" $scenarioBookPanel "delegate \{ return LaunchCoordinator; \}.*delegate \{ return DraftMetadataEditService; \}.*delegate \{ return ImportService; \}" "opening the scenario library must pass deferred tool and launch dependencies instead of resolving the runtime/editor graph eagerly."
Assert-Contains "scenario book deferred action dependencies" $scenarioBookActions "Func<ScenarioLaunchCoordinator> _launchCoordinatorFactory.*Func<ScenarioDraftMetadataEditService> _draftMetadataEditServiceFactory.*Func<ScenarioPackageImportService> _importServiceFactory" "the browser action service must resolve heavyweight launch, editor, and import services only when their actions are invoked."
Assert-Contains "scenario book deferred import dependency" $scenarioBookDataSource "Func<ScenarioPackageImportService> _importServiceFactory.*private ScenarioPackageImportService ImportService" "the normal library data source must not construct import tooling until the downloads view needs it."
Assert-Contains "scenario book deferred action synchronization" $scenarioBookActions "ResolveDeferred<T>.*lock \(_dependencySync\).*service = Require\(factory\(\), name\)" "deferred action services must be initialized and published under one synchronization boundary."
Assert-Contains "scenario book import thread boundary" $scenarioBookDataSource "ScenarioPackageImportService importService = ImportService;.*ThreadPool\.QueueUserWorkItem.*result = importService\.Scan\(\)" "composition-backed import services must resolve on the Unity caller thread before filesystem work crosses to the worker pool."
Assert-Contains "scenario book import safe publication" $scenarioBookDataSource "private volatile ScenarioPackageImportService _importService.*lock \(_importServiceSync\)" "the double-checked deferred import service must use volatile publication under the synchronization boundary."
Assert-Contains "scenario book import enqueue recovery" $scenarioBookDataSource "ScenarioPackageImportService importService = ImportService;.*bool queued;.*ResetFailedImportRefresh\(requestVersion\).*Could not queue the downloaded scenario scan" "dependency resolution and worker-enqueue failures must not leave import refresh permanently marked as running."
Assert-Contains "scenario book panel import synchronization" $scenarioBookPanel "private readonly object _importServiceSync.*lock \(_importServiceSync\).*_importService = new ScenarioPackageImportService" "the panel's deferred import service must be initialized and published under synchronization."
...
Assert-Contains "editor patches activate from scenario book" $scenarioBookActions "private static void EnsureEditorRuntime\(string trigger\).*ShelteredDeferredPatchTriggers\.ApplyEditorDeferred\(trigger\)" "creating or opening a draft from the scenario book must apply editor patches before queuing authoring."
Assert-Contains "editor patches activate from vanilla selection" $scenarioSelectionBrowserController 'ApplyEditorDeferred\("ScenarioSelectionPanel Add New"\).*QueueNewDraft' "the vanilla scenario-selection Add New route must apply editor patches before queuing authoring."
Assert-Contains "editor deferred activation order" $shelteredDeferredPatchTriggers "ApplyEditorDeferred\(string trigger\).*ApplyDeferredPatchGroup\(PatchStartupTiming\.EditorDeferred, trigger\).*ScenarioAuthoringInputActions\.EnsureRegistered\(\).*ScenarioAuthoringRuntimeDriver\.EnsureCreated\(\)" "editor patches must complete before the authoring input and runtime driver become active."
Assert-Contains "editor harness compatibility seam" $shelteredDeferredPatchTriggers "ApplyEditorDeferred\(string trigger\).*EnsureEditorRuntime\(trigger\).*public static void EnsureEditorRuntime\(string trigger\).*ApplyDeferredPatchGroup\(PatchStartupTiming\.EditorDeferred, trigger\)" "the legacy in-game harness seam must apply deferred patches before starting the authoring runtime."
Assert-Contains "editor-only boot patch deferral" $shelteredCustomScenarioPatches '"ScenarioAuthoringGlobalUiIsolation".*StartupTiming = PatchStartupTiming\.EditorDeferred.*"ScenarioAuthoringPauseOwnership".*StartupTiming = PatchStartupTiming\.EditorDeferred.*"ScenarioAuthoringSimulationFreeze".*StartupTiming = PatchStartupTiming\.EditorDeferred.*"ScenarioDynamicFamilyUiReadiness".*StartupTiming = PatchStartupTiming\.EditorDeferred' "authoring-only global UI, pause, simulation, and family UI guards must stay off the normal boot-critical Harmony path."
...
Assert-Contains "installed-copy semantic descriptions" $scenarioEntryFlow 'editableDefinition != null && !string\.IsNullOrEmpty\(editableDefinition\.Description\).*editableDefinition\.Description.*Safe\(entry\.Description, "No scenario description was provided\."\)' "copy-an-installed-scenario cards must explain the scenario with its authored description instead of a structural count report."
