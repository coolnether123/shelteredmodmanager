...
Assert-Contains "scenario book hidden visual prewarm" $scenarioBookPanel "TryPrepareVisual\(\).*PrepareVisual\(root\).*root\.SetActive\(false\).*_preparedInstance = root" "menu-idle prewarm must build and retain hidden scenario-book chrome before the user opens it."
