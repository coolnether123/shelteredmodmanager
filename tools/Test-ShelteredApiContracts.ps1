...
$condensePromptDialog = Read-RepoFile "ShelteredAPI\Saves\Paging\CondensePromptDialog.cs"
$nguiModalPrimitives = Read-RepoFile "ShelteredAPI\UI\NguiModalPrimitives.cs"
...
Assert-Contains "vanilla page refresh registry reuse" $saveRegistry "return ExpandedVanillaSaves\.GetBySlot\(slotNumber\)" "page-zero vanilla refreshes must reuse the shared Standard registry instead of reparsing every save directory on each page turn."
...
Assert-Contains "modal button centers" $nguiModalPrimitives 'background\.pivot = UIWidget\.Pivot\.Center;.*label\.pivot = UIWidget\.Pivot\.Center;.*label\.transform\.localPosition = Vector3\.zero;' "shared modal buttons must restore caption position after NGUI font sizing so the face and text share the same visual center."
Assert-Contains "condense prompt centers checkbox" $condensePromptDialog 'checkboxBg\.GetComponent<UITexture>\(\)\.pivot = UIWidget\.Pivot\.Center;.*checkmark\.pivot = UIWidget\.Pivot\.Center;.*checkboxLabel\.pivot = UIWidget\.Pivot\.Left;' "the organize-save prompt checkbox controls must use explicit pivots so they do not drift toward the bottom-right."
...
Assert-Contains "library page preparation keeps controls" $scenarioBookRenderer "BuildPreparedPage\([\s\S]*?pageCount,[\s\S]*?view == ScenarioBookBrowserViewKind\.Types[\s\S]*?BuildLibraryPageControls\(library, pageIndex, pageCount\)" "prepared library pages must retain their local pager controls when the page swap uses the warm cache."
Assert-NotContains "library page swaps use prepared pages" $scenarioBookPanel "pageCount <= 1 \|\| _view == ScenarioBookBrowserViewKind\.Types" "library pagination must prebuild adjacent pages instead of rebuilding the visible NGUI spread on every page turn."
