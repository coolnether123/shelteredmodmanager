...
Four Person Expeditions, Better AI Queue, Family Expansion, Lifespan, and Bunker Random Location use one-call completion-backed interaction transactions (`four-person-expedition`, `better-ai-queue-persistence`, `family-persistence`, `lifespan-persistence`, and `bunker-persistence`). The graph scenario ID is the same ID passed to the transaction step, so the runner cannot silently execute a different scenario. Family persistence fingerprints include Lifespan inputs, Bunker persistence fingerprints include Expanded Map Sizes inputs, and expedition fingerprints include the Bunker dependency. A provider change therefore reselects only the dependent transaction plus its normal downstream gates; unrelated heavy matrices remain reusable.

`-Execute` may build and package selected work. `-AllowHeavy` is required for state-heavy gameplay fixtures. A skipped, blocked, not-configured, or failed selected gate causes a nonzero exit and prevents packaging/promotion. Passed gates write receipts under `release/2.0/evidence/incremental-gates` by default. Reuse requires the same gate/configuration and a matching scoped fingerprint. A gameplay fingerprint includes only the selected scenario `harnessInputs`, the transaction runner, the game assembly/load order, shared runtime frameworks (`ModAPI`, `PluginHarmony`, `PluginInitializer`), and the `runtimeModDirectories` of the gate owner and its declared dependency owners. It does not hash the whole harness binary, every installed mod, or the complete graph for every gate; only the release-graph self-test fingerprints the complete graph. Use `-NoEvidenceReuse` for an intentional full rerun. `-Stable` adds the live SMM promotion gate, which intentionally fails closed until the issued Nexus OAuth identity and real-account evidence exist. This tool never commits, pushes, publishes, edits mod repositories, or edits `ShelteredAgentInterface`.
