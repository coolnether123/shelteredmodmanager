...
$inputCapture = Read-Source 'ShelteredScenarioEditor\Presentation\Authoring\Shell\ScenarioAuthoringInputCaptureService.cs'
...
$scenarioRuntimeFacade = Read-Source 'ShelteredAPI\Scenarios\Application\Runtime\Preview\ScenarioRuntimeFacadeMappers.cs'
$spriteRuntime = Read-Source 'ShelteredAPI\Scenarios\Infrastructure\Assets\ScenarioSpriteRuntimeMutationService.cs'
$spriteTargetAdapter = Read-Source 'ShelteredScenarioEditor\Infrastructure\Assets\ScenarioEditorSpriteRuntimeResolver.cs'
...
$camera = Read-Source 'ShelteredScenarioEditor\Infrastructure\Unity\ScenarioAuthoringEditorCameraService.cs'
$spriteCatalog = Read-Source 'ShelteredScenarioEditor\Infrastructure\Assets\ScenarioSpriteCatalogService.cs'
$spriteReferenceLibrary = Read-Source 'ShelteredAPI\Scenarios\Infrastructure\Assets\ScenarioSpriteReferenceLibrary.cs'
$toolRail = Read-Source 'ShelteredScenarioEditor\Presentation\Authoring\Shell\Rendering\ScenarioAuthoringShellToolRailImguiRenderer.cs'
$bounds = Read-Source 'ModAPI\Inspector\BoundsHighlighter.cs'
$interaction = Read-Source 'ShelteredScenarioEditor\Infrastructure\Unity\ScenarioVanillaInteractionRuntimeService.cs'
$editorController = Read-Source 'ShelteredScenarioEditor\Application\Authoring\ScenarioEditorController.cs'
$authoringBackend = Read-Source 'ShelteredScenarioEditor\Application\Authoring\ScenarioAuthoringBackendService.cs'
$shellWindowRenderer = Read-Source 'ShelteredScenarioEditor\Presentation\Authoring\Shell\Rendering\ScenarioAuthoringShellWindowImguiRenderer.cs'
$shellRenderer = Read-Source 'ShelteredScenarioEditor\Presentation\Authoring\Shell\ScenarioAuthoringShellImguiRenderModule.cs'
$surfaceGate = Read-Source 'ShelteredScenarioEditor\Presentation\Authoring\Shell\Rendering\ScenarioAuthoringShellVisualSurfaceGate.cs'
$layoutService = Read-Source 'ShelteredScenarioEditor\Presentation\Authoring\Shell\ScenarioAuthoringLayoutService.cs'
$timelineRibbon = Read-Source 'ShelteredScenarioEditor\Presentation\Authoring\Shell\Rendering\ScenarioAuthoringShellTimelineRibbonImguiRenderer.cs'
$entryFlow = Read-Source 'ShelteredScenarioEditor\Application\Authoring\ScenarioAuthoringEntryFlowService.cs'
$assetBrowserRenderer = Read-Source 'ShelteredScenarioEditor\Presentation\Authoring\Shell\Rendering\ScenarioAuthoringShellAssetBrowserImguiRenderer.cs'
...
Assert-Match 'pixel edits route to current frame preview' $spriteAuthoring 'ApplyCustomEditorPreview.*IsAnimationEditor\(\).*_previewHost\.TryPreviewRuntimeSpriteFrame'
Assert-Match 'pixel frame preview facade reaches runtime mutation' $scenarioRuntimeFacade 'TryPreviewRuntimeSpriteFrame.*TryResolveRuntimeTarget.*ScenarioSpriteRuntimeMutationService\.TryPreviewEditedFrame'
...
Assert-Match 'sprite resolution prefers durable target path over transient runtime proxy' $spriteTargetAdapter 'string targetPath = ResolveTargetPath\(authoringTarget\);.*ShelteredScenarioRuntime\.TryResolveRuntimeSpriteTarget\(.*targetPath.*return true;.*Transform runtimeTransform = ResolveTransform\(authoringTarget\);'
Assert-Match 'sprite resolution reconciles descendant hit proxies through canonical durable paths' $spriteTargetAdapter 'Transform runtimeTransform = ResolveTransform\(authoringTarget\);.*while \(current != null\).*ShelteredScenarioRuntime\.GetTransformPath\(current\), targetPath.*transform = current.*current = current\.parent.*transform = runtimeTransform'
...
Assert-Match 'saved sprite rules retain exact rendered children through canonical durable paths' $spriteTargetResolver 'TargetPath = ScenarioTransformPath\.Build\(spriteRenderer\.transform\).*Transform = spriteRenderer\.transform.*TargetPath = ScenarioTransformPath\.Build\(ui2DSprite\.transform\).*TargetPath = ScenarioTransformPath\.Build\(particleRenderer\.transform\)'
if ([regex]::IsMatch(($spriteTargetAdapter + $spriteTargetResolver), '(private|internal|public)\s+(static\s+)?string\s+(Build|Get)TransformPath\s*\(', [Text.RegularExpressions.RegexOptions]::Singleline)) {
    $failures.Add('sprite resolver does not restore a local transform-path builder')
}
...
Assert-Match 'pixel editor uses vanilla target geometry rather than candidate geometry' $spriteAuthoring 'Sprite targetGeometrySprite = _previewSession != null && _previewSession\.BaselineSprite != null.*CreateEditableTexture\(sourceSprite, targetGeometrySprite\).*CreatePreviewSprite\(editableTexture, targetGeometrySprite\).*CreateEditableTexture\(sourceSprite, targetGeometrySprite\)'
Assert-Match 'pixel editor normalizes candidate pixels to vanilla canvas dimensions' $spriteAuthoring 'CreateEditableTexture\(Sprite source, Sprite targetGeometry = null\).*Rect targetRect = targetGeometry != null \? targetGeometry\.rect : sourceRect.*sourceWidth == width && sourceHeight == height.*Color\[\] targetPixels = new Color\[width \* height\]'
Assert-Match 'sprite alignment fits replacements without transform scaling' $spriteSwapRenderer 'baselineWorldWidth = baselineRect\.width / baselinePixelsPerUnit.*baselineWorldHeight = baselineRect\.height / baselinePixelsPerUnit.*alignedPixelsPerUnit = Mathf\.Max\(widthPixelsPerUnit, heightPixelsPerUnit\).*Sprite\.Create\(.*alignedPixelsPerUnit\)'
...
Assert-Match 'asset cards reserve detail for selected pane' $assetBrowserRenderer 'Selected Asset pane owns technical/source.*DrawCandidateCard\(cardRect, item\.Action, armPlacementOnCardClick, true, true, true\)'
