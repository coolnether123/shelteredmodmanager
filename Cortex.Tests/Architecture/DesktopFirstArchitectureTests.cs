...
namespace Cortex.Tests.Architecture
{
    public sealed class DesktopFirstArchitectureTests
    {
        ...
        public void DesktopAvaloniaHost_UsesSharedShellContracts_AndStructuredLogging()
        {
            ...
            var programText = ArchitectureTestEnvironment.ReadRepoFile("Cortex.Host.Avalonia", "Program.cs");
            ...
            Assert.Contains("DesktopHostLaunchCoordinator.GetActiveSession()", appText);
            Assert.DoesNotContain("--pipe-name", appText);
            Assert.DoesNotContain("CORTEX_DESKTOP_BRIDGE_PIPE_NAME", appText);
            Assert.Contains("DesktopHostLaunchCoordinator", programText);
            Assert.Contains("new DesktopHostLaunchCoordinator().Run(args)", programText);
            ...
            Assert.Contains("DesktopDefaultHostProfile", bundlePolicyText);
            Assert.Contains("Harmony", bundlePolicyText);
            Assert.Contains("roslyn", bundlePolicyText);
            Assert.Contains("tabby", bundlePolicyText);
            ...
            Assert.Contains("BundleProfile", sessionText);
            ...
            Assert.Contains("Desktop Host Bundle", statusViewText);
        ...
        public void DesktopFirstDirection_IsExplicitAcrossArchitectureDocs()
        {
            ...
            Assert.Contains("artifacts\\bundles\\Desktop\\", buildTopologyGuideText);
            ...
            Assert.Contains("CORTEX_DESKTOP_BUNDLE_ROOT", hostGuideText);
